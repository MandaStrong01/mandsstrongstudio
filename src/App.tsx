...
const Card = (x) => ({ background:"#1C1F25", border:"1px solid "+GOLDDIM, borderRadius:10, padding:18, ...(x||{}) });
...
function ProjectHistoryModal({ onClose, onResume, initialTab }) {
  ...
  return (
    <div style={{position:"fixed",inset:0,zIndex:1200,background:"rgba(0,0,0,0.96)",display:"flex",alignItems:"center",justifyContent:"center"}}>
      <div style={{width:"min(620px,95vw)",background:"#14161A",border:"2px solid "+SIGNAL,maxHeight:"85vh",display:"flex",flexDirection:"column"}}>
        <div style={{background:"#1C1F25",borderBottom:"1px solid "+GOLDDIM+"",padding:"16px 22px",display:"flex",justifyContent:"space-between",alignItems:"center",flexShrink:0}}>
          <div>
          ...
          <button onClick={onClose} style={{background:"none",border:"1px solid "+GOLDDIM,color:GOLD,width:30,height:30,cursor:"pointer",fontSize:15}}>â</button>
        </div>
        <div style={{display:"flex",borderBottom:"1px solid "+GOLDDIM,flexShrink:0}}>
          <button onClick={()=>setTab("in_progress")} style={{flex:1,background:tab==="in_progress"?"#22262D":"transparent",border:"none",borderBottom:tab==="in_progress"?"2px solid "+SIGNAL:"none",color:tab==="in_progress"?GOLD:DIM,padding:"12px",cursor:"pointer",fontSize:12,fontWeight:600,letterSpacing:0.2,fontFamily:"'Archivo',system-ui,sans-serif"}}>â³ open project ({inProgressCount})</button>
          <button onClick={()=>setTab("completed")} style={{flex:1,background:tab==="completed"?"#22262D":"transparent",border:"none",borderBottom:tab==="completed"?"2px solid "+SIGNAL:"none",color:tab==="completed"?GOLD:DIM,padding:"12px",cursor:"pointer",fontSize:12,fontWeight:600,letterSpacing:0.2,fontFamily:"'Archivo',system-ui,sans-serif"}}>â my projects ({completedCount})</button>
        </div>
        <div style={{flex:1,overflowY:"auto",padding:18}}>
          {filtered.length===0?(
            <div style={{textAlign:"center",padding:"40px 20px",color:GOLDDIM}}>
              <div style={{fontSize:34,marginBottom:10}}>{tab==="in_progress"?"â³":"â"}</div>
            ...
            return (
              <div key={i} style={{background:"#1C1F25",border:"1px solid "+GOLDDIM,padding:"12px 16px",marginBottom:10,display:"flex",alignItems:"center",gap:12}}>
                <div style={{flex:1}}>
                  ...
                  <div style={{color:DIM,fontSize:10,letterSpacing:0}}>{h.date} Â· Page {h.page} Â· {h.assetCount} asset{h.assetCount!==1?"s":""}</div>
                  {h.note&&<div style={{color:WHITE,fontSize:11,marginTop:4,fontStyle:"italic"}}>{h.note}</div>}
                </div>
                <div style={{display:"flex",gap:6,flexShrink:0}}>
                  <button onClick={()=>onResume(h)} style={{background:"#1C1F25",border:"none",color:"#000",padding:"8px 18px",cursor:"pointer",fontSize:11,fontWeight:600,letterSpacing:0.2,fontFamily:"'Archivo',system-ui,sans-serif"}}>{tab==="completed"?"Revisit":"Continue"}</button>
                  <button onClick={()=>del(originalIdx)} style={{background:"none",border:"1px solid #ef4444",color:"#ef4444",padding:"5px 10px",cursor:"pointer",fontSize:10,fontWeight:600,fontFamily:"'Archivo',system-ui,sans-serif"}}>â</button>
                </div>
              </div>
            );
          })}
...
function SaveSessionModal({ onClose, onSave, currentPage, assetCount }) {
  ...
  const inp2={width:"100%",background:"#1C1F25",border:"1px solid "+GOLDDIM,padding:"9px 12px",color:WHITE,fontSize:13,outline:"none",boxSizing:"border-box",fontFamily:"'Archivo',system-ui,sans-serif"};
  return (
    <div style={{position:"fixed",inset:0,zIndex:1200,background:"rgba(0,0,0,0.92)",display:"flex",alignItems:"center",justifyContent:"center"}}>
      <div style={{width:"min(440px,92vw)",background:"#14161A",border:"2px solid "+SIGNAL,padding:22}}>
        ...
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:6,marginBottom:16}}>
          <button onClick={()=>setStatus("in_progress")} style={{background:status==="in_progress"?GOLD:"#1C1F25",border:"1px solid "+(status==="in_progress"?"#000":GOLDDIM),color:status==="in_progress"?"#000":WHITE,padding:"9px",cursor:"pointer",fontSize:11,fontWeight:600,letterSpacing:0.2,fontFamily:"'Archivo',system-ui,sans-serif"}}>â³ in progress</button>
          <button onClick={()=>setStatus("completed")} style={{background:status==="completed"?GOLD:"#1C1F25",border:"1px solid "+(status==="completed"?"#000":GOLDDIM),color:status==="completed"?"#000":WHITE,padding:"9px",cursor:"pointer",fontSize:11,fontWeight:600,letterSpacing:0.2,fontFamily:"'Archivo',system-ui,sans-serif"}}>Completed</button>
        </div>
...
function QAMenu({ go, onClose, user }) {
  return (
    <div style={{position:"fixed",inset:0,zIndex:1000,display:"flex"}}>
      <div style={{width:256,background:"#14161A",borderRight:"1px solid "+GOLDDIM+"",height:"100vh",overflowY:"auto",padding:18}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:18}}>
          ...
          <button onClick={onClose} style={{background:"none",border:"none",color:GOLD,fontSize:20,cursor:"pointer"}}>â</button>
        </div>
        ...
        <div style={{background:"#1C1F25",border:"1px solid "+GOLDDIM,padding:"7px 10px",marginBottom:14,textAlign:"center"}}>
...
function Header({ go, setMenu }) {
  return (
    <header style={{position:"sticky",top:0,zIndex:500,background:"#1C1F25",borderBottom:"1px solid "+GOLDDIM+"",padding:"0 16px",height:52,display:"flex",alignItems:"center",gap:12}}>
      <button onClick={()=>setMenu(true)} style={{background:"none",border:"1px solid "+GOLDDIM,color:GOLD,width:34,height:34,cursor:"pointer",fontSize:16,flexShrink:0}}>â°</button>
      ...
      <div style={{flex:1,display:"flex",alignItems:"center",justifyContent:"center"}}>
        <div style={{color:GOLD,fontSize:11,letterSpacing:0.2,whiteSpace:"nowrap",overflow:"hidden",textOverflow:"ellipsis",fontWeight:500}}>
          Cinema intelligence platform &nbsp;Â·&nbsp; 600+ AI tools &nbsp;Â·&nbsp; 8K export &nbsp;Â·&nbsp; films up to 3 hours
        </div>
      </div>
...
function Footer({ page, go, onSave, onHistory }) {
  return (
    <footer style={{position:"fixed",bottom:0,left:0,right:0,zIndex:400,background:"#1C1F25",borderTop:"1px solid "+GOLDDIM+"",padding:"6px 20px 8px",display:"flex",flexDirection:"column",gap:4}}>
      <div style={{textAlign:"center"}}>
        <span style={{color:GOLD,fontSize:11,letterSpacing:0,fontWeight:500}}>MANDASTRONG STUDIO Â· PROFESSIONAL CINEMA SYNTHESIS Â· MandaStrong1.Etsy.com</span>
        {page===1&&<span style={{color:GOLD,fontSize:11,letterSpacing:0,fontWeight:500,opacity:0.75}}> Â· created 2025</span>}
      </div>
      <div style={{display:"flex",alignItems:"center",justifyContent:"center",gap:10,flexWrap:"wrap"}}>
        ...
        <button onClick={onHistory} style={{background:"#1C1F25",border:"1px solid "+GOLDDIM,color:GOLD,padding:"5px 14px",cursor:"pointer",fontSize:11,fontWeight:600,letterSpacing:0.2,fontFamily:"'Archivo',system-ui,sans-serif"}}>My projects</button>
...
function ToolCard({ name, onOpen }) {
  return (
    <div onClick={()=>onOpen(name)}
      style={{background:"#1C1F25",border:"1px solid "+GOLDDIM,padding:"14px 12px",cursor:"pointer",transition:"all .15s",minHeight:56,display:"flex",alignItems:"center"}}
...
function ToolPanel({ tool, onClose, onSave }) {
  ...
  const inp = {width:"100%",background:"#1C1F25",border:"1px solid "+GOLDDIM,padding:"9px 12px",color:WHITE,fontSize:14,outline:"none",boxSizing:"border-box",fontFamily:"'Archivo',system-ui,sans-serif"};
  ...
  return (
    <div style={{position:"fixed",inset:0,zIndex:900,background:"rgba(0,0,0,0.92)",display:"flex",alignItems:"center",justifyContent:"center"}}>
      <div style={{width:"min(600px,95vw)",background:"#14161A",border:"1px solid "+GOLDDIM,padding:26,maxHeight:"92vh",overflowY:"auto"}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:18}}>
          ...
          <div>
            ...
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8,marginBottom:16}}>
              {STOCK_VOICES.map(v=>(
                <div key={v.id} onClick={()=>setSelVoice(v.id)}
                  style={{background:"#1C1F25",border:"2px solid "+selVoice===v.id?GOLD:GOLDDIM,padding:"10px 12px",cursor:"pointer",boxShadow:selVoice===v.id?"0 0 12px "+GOLD+"44":"none"}}>
            ...
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8,marginBottom:result?14:0}}>
              <button onClick={runAI} disabled={loading||!describe.trim()} style={{...G("gold",false),padding:"12px",opacity:loading||!describe.trim()?0.5:1}}>
                {loading?"â³ GENERATING...":"AI format & speak"}
              </button>
          ...
          <div style={{marginBottom:14}}>
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8,marginBottom:4}}>
              <button onClick={()=>photoRef.current&&photoRef.current.click()}
                style={{background:"#1C1F25",border:"2px solid "+SIGNAL,color:GOLD,padding:"20px 8px",cursor:"pointer",fontSize:12,fontWeight:600,letterSpacing:0,fontFamily:"'Archivo',system-ui,sans-serif"}}>
                Upload photo
              </button>
              <button onClick={()=>fileRef.current&&fileRef.current.click()}
                style={{background:"#1C1F25",border:"1px solid "+GOLDDIM,color:WHITE,padding:"20px 8px",cursor:"pointer",fontSize:12,fontWeight:600,letterSpacing:0,fontFamily:"'Archivo',system-ui,sans-serif"}}>
                Upload file
              </button>
            </div>
            <a href="https://photos.google.com" target="_blank" rel="noopener noreferrer"
              style={{display:"block",background:"#1C1F25",border:"1px solid "+GOLDDIM,color:GOLDDIM,padding:"8px",textAlign:"center",fontSize:10,fontWeight:600,letterSpacing:0.2,textDecoration:"none",fontFamily:"'Archivo',system-ui,sans-serif",marginBottom:4}}>
        ...
        {mode==="ai"&&tool==="Script to Movie"&&(
          <div style={{marginBottom:14}}>
            ...
            <button onClick={runAI} disabled={loading||!describe.trim()} style={{...G("gold",false),width:"100%",padding:"13px",opacity:loading||!describe.trim()?0.5:1,fontSize:13,letterSpacing:0.2,marginBottom:8}}>{loading?"â³ CREATING...":"Write script"}</button>
            <button onClick={()=>{
            ...
            }} style={{width:"100%",background:GOLD,border:"none",color:"#000",padding:"14px",cursor:"pointer",fontSize:13,fontWeight:600,letterSpacing:0.2,fontFamily:"'Archivo',system-ui,sans-serif"}}>Wire into render â drive the video generator</button>
            {s2mWired&&<div style={{color:"#22c55e",fontSize:11,fontWeight:600,letterSpacing:0,marginTop:10,textAlign:"center"}}>â WIRED â Page 8 will use your Producer, Describe &amp; Production notes.</div>}
            {result&&(
        ...
        {mode==="ai"&&tool!=="Script to Movie"&&(
          <div style={{marginBottom:14}}>
            ...
            <button onClick={runAI} disabled={loading||!describe.trim()} style={{...G("gold",false),marginTop:8,width:"100%",padding:"14px",opacity:loading||!describe.trim()?0.5:1,fontSize:13,letterSpacing:0.2}}>
              {loading?"â³ CREATING...":isVideoTool?"Create video package":isImageTool?"Create image prompt":isWritingTool?"Write script":"AI create"}
            </button>
            {result&&(
              <div style={{marginTop:14}}>
                <textarea value={result} onChange={e=>setResult(e.target.value)} style={{...inp,height:140,resize:"none",lineHeight:1.7}}/>
...
function WritingBoxes({ onSave }) {
  const BOXES=[
    {key:"producer",title:"PRODUCER",icon:"",hint:"Vision, tone, casting, the feeling the film should leave behind.",ph:"Producer's directions â vision, mood, casting, overall intent..."},
    {key:"describe",title:"DESCRIBE",icon:"",hint:"Describe the film scene by scene â what happens, what we see.",ph:"Describe your film â scene by scene, what happens on screen..."},
    {key:"production",title:"PRODUCTION NOTES",icon:"",hint:"Shots, camera moves, lighting, locations, timing.",ph:"Production notes â shots, camera moves, lighting, locations, timing..."},
  ];
  ...
  const ta={width:"100%",background:"#1C1F25",border:"1px solid "+GOLDDIM,padding:"12px 14px",color:WHITE,fontSize:13,outline:"none",boxSizing:"border-box",fontFamily:"'Archivo',system-ui,sans-serif",lineHeight:1.8,height:150,resize:"vertical"};
  return (
    <div style={{padding:"0 12px 16px"}}>
      <div style={{color:GOLD,fontSize:13,letterSpacing:0.2,fontWeight:600,margin:"6px 2px 4px"}}>Script to movie</div>
      <div style={{color:GOLDDIM,fontSize:11,letterSpacing:0,margin:"0 2px 12px"}}>Fill the three boxes, then WIRE INTO RENDER â the video generator on Page 8 uses them to drive every scene. Each box also saves to your Media Library &amp; timeline.</div>
      <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(260px,1fr))",gap:12}}>
        {BOXES.map(b=>(
          <div key={b.key} style={{background:"#14161A",border:"2px solid "+SIGNAL,padding:"14px 16px",display:"flex",flexDirection:"column"}}>
      ...
      <button onClick={wireToRender} disabled={wiring} style={{marginTop:14,width:"100%",background:GOLD,border:"none",color:"#000",padding:"15px",cursor:"pointer",fontSize:14,fontWeight:600,letterSpacing:0.2,fontFamily:"'Archivo',system-ui,sans-serif"}}>{wiring?"â³ WIRING...":"Wire into render â drive the video generator"}</button>
      {wired&&<div style={{color:"#22c55e",fontSize:12,fontWeight:600,letterSpacing:0,marginTop:10,textAlign:"center"}}>â WIRED â Page 8 will now use your Producer, Describe &amp; Production notes on every scene.</div>}
    </div>
  );
}
    ...
    <div style={{...Sp}}>
        ...
        <div style={{display:"flex",alignItems:"center",gap:8}}>
          <div style={{position:"relative"}}>
            <input value={search} onChange={e=>setSearch(e.target.value)} placeholder={"Search "+tools.length+" tools..."}
              style={{background:"#1C1F25",border:"1px solid "+GOLDDIM,padding:"7px 12px 7px 28px",color:WHITE,fontSize:13,outline:"none",width:200}}/>
            <span style={{position:"absolute",left:8,top:"50%",transform:"translateY(-50%)",color:GOLD}}></span>
            {search&&<button onClick={()=>setSearch("")} style={{position:"absolute",right:7,top:"50%",transform:"translateY(-50%)",background:"none",border:"none",color:GOLD,cursor:"pointer",padding:0}}>â</button>}
          </div>
      ...
      {title==="WRITING TOOLS"&&(
        <div style={{padding:"0 12px 12px"}}>
          <div style={{background:"#14161A",border:"2px solid "+SIGNAL,padding:"16px 20px",display:"flex",alignItems:"center",justifyContent:"space-between",flexWrap:"wrap",gap:12}}>
            <div>
            ...
            <div style={{display:"flex",gap:10}}>
              <button onClick={()=>{
                ...
                style={{background:"#1C1F25",border:"none",color:"#000",padding:"12px 24px",cursor:"pointer",fontSize:12,fontWeight:600,letterSpacing:0.2}}>
                Open project
              </button>
            </div>
          </div>
...
function RecordYourOwnSong({ onRecorded }) {
  ...
  return recording?(
    <div style={{display:"flex",alignItems:"center",gap:10,background:"#1a0000",border:"1px solid #ef4444",padding:"10px 14px",marginTop:8}}>
      ...
      <span style={{color:"#ef4444",fontWeight:600,fontSize:12,letterSpacing:0.2,flex:1}}>Recording â {fmt(recTime)}</span>
    ...
    <button onClick={start} style={{width:"100%",background:"#1C1F25",border:"none",color:"#fff",padding:"10px 14px",cursor:"pointer",fontSize:12,fontWeight:600,letterSpacing:0.2,marginTop:8,fontFamily:"'Archivo',system-ui,sans-serif"}}>
...
function MusicVideoStudio({ onClose, onSave }) {
    ...
    try {
      ...
      if(audioFile){
        try{
          ...
          if(config.stereo!==false && audioCtx.createStereoPanner){
            addLog("Stereo sound ON â baking full stereo field into the film");
      ...
      if(config.refMedia){
        try{
          ...
          if(refImgEl) addLog("Reference image loaded â Reality Engine base active");
        }catch(e){}
      }
      ...
      addLog("â "+config.title+" complete â "+(blob.size/1024/1024).toFixed(1)+"MB Â· "+Math.round(totalDur)+"s");
  ...
  const inp = {width:"100%",background:"#1C1F25",border:"1px solid "+GOLDDIM,padding:"9px 12px",color:WHITE,fontSize:13,outline:"none",fontFamily:"'Archivo',system-ui,sans-serif",boxSizing:"border-box"};
    ...
    <div style={{display:"flex",flexWrap:"wrap",gap:5,marginBottom:4}}>
      {arr.map(item=>(
        <button key={item} onClick={()=>set(k,item)}
          style={{background:config[k]===item?GOLD:"#1C1F25",border:"1px solid "+(config[k]===item?"#000":GOLDDIM),color:config[k]===item?"#000":WHITE,padding:"4px 10px",cursor:"pointer",fontSize:11,fontWeight:600,letterSpacing:0}}>
          {item}
        </button>
      ))}
    </div>
    ...
    <div style={{display:"flex",flexWrap:"wrap",gap:5,marginBottom:4}}>
      {arr.map(item=>(
        <button key={item} onClick={()=>tog(k,item)}
          style={{background:config[k].includes(item)?GOLD:"#1C1F25",border:"1px solid "+(config[k].includes(item)?"#000":GOLDDIM),color:config[k].includes(item)?"#000":WHITE,padding:"4px 10px",cursor:"pointer",fontSize:11,fontWeight:600,letterSpacing:0}}>
          {item}
        </button>
      ))}
    </div>
  ...
  return (
    <div style={{position:"fixed",inset:0,zIndex:1100,background:"rgba(0,0,0,0.98)",display:"flex",alignItems:"center",justifyContent:"center"}}>
      <div style={{width:"min(960px,98vw)",height:"min(92vh,860px)",background:"#14161A",border:"2px solid "+SIGNAL,display:"flex",flexDirection:"column",overflow:"hidden"}}>

        {/* Header */}
        <div style={{background:"#1C1F25",borderBottom:"1px solid "+GOLDDIM+"",padding:"14px 22px",display:"flex",justifyContent:"space-between",alignItems:"center",flexShrink:0}}>
          <div>
            ...
            <div style={{color:WHITE,fontSize:10,letterSpacing:0.2,marginTop:2}}>Professional music video production Â· AI powered Â· self-contained</div>
          </div>
          <button onClick={onClose} style={{background:"none",border:"1px solid "+GOLDDIM,color:GOLD,width:32,height:32,cursor:"pointer",fontSize:16}}>â</button>
        </div>
        ...
        <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",borderBottom:"1px solid "+GOLDDIM+"",flexShrink:0}}>
          {steps.map((s,i)=>(
            <button key={i} onClick={()=>setStep(i+1)}
              style={{background:step===i+1?"#1C1F25":"none",border:"none",borderBottom:step===i+1?"2px solid "+SIGNAL:"2px solid transparent",color:step===i+1?GOLD:WHITE,padding:"11px 6px",cursor:"pointer",fontSize:11,fontWeight:600,letterSpacing:0.2}}>
              {s}
            </button>
          ))}
        </div>
        ...
        <div style={{flex:1,display:"grid",gridTemplateColumns:videoUrl?"1fr 1fr":"1fr",overflow:"hidden"}}>
          ...
          <div style={{overflowY:"auto",padding:"16px 20px",borderRight:videoUrl?"1px solid "+GOLDDIM:"none"}}>
            ...
            {step===1&&(
              <div>
                ...
                {label("Upload your song â or record it below")}
                <div style={{background:"#1C1F25",border:"2px dashed "+(audioFile?GOLD:GOLDDIM),padding:"18px 12px",cursor:"pointer",transition:"all .2s"}}
                  onClick={()=>audioInputRef.current&&audioInputRef.current.click()}
                  onDragOver={e=>{e.preventDefault();e.currentTarget.style.borderColor=GOLD;e.currentTarget.style.background="#1C1F25";}}
                  ...
                  <div style={{color:audioFile?"#22c55e":GOLD,fontWeight:600,fontSize:13,letterSpacing:0.2,textAlign:"center"}}>
                    {audioFile?"â "+audioName:"Drag & drop your song here"}
                  </div>
                  <div style={{color:GOLDDIM,fontSize:10,marginTop:4,textAlign:"center",letterSpacing:0}}>{audioFile?"Tap to replace":"or tap to browse â MP3 Â· WAV Â· M4A"}</div>
                </div>
                ...
                <div onClick={()=>set("stereo",!config.stereo)} style={{display:"flex",alignItems:"center",gap:10,marginTop:14,padding:"10px 12px",background:"#1C1F25",border:"1px solid "+(config.stereo?GOLD:GOLDDIM),cursor:"pointer"}}>
                  <div style={{width:20,height:20,borderRadius:4,border:"2px solid "+(config.stereo?GOLD:GOLDDIM),background:config.stereo?GOLD:"transparent",color:"#000",fontWeight:600,fontSize:14,display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0}}>{config.stereo?"â":""}</div>
                ...
                <div style={{display:"flex",gap:6,flexWrap:"wrap"}}>
                  {["16:9","9:16 (Vertical)","1:1 (Square)","2.39:1 (Cinematic)"].map(r=>(
                    <button key={r} onClick={()=>set("aspectRatio",r)}
                      style={{background:config.aspectRatio===r?GOLD:"#1C1F25",border:"1px solid "+(config.aspectRatio===r?"#000":GOLDDIM),color:config.aspectRatio===r?"#000":WHITE,padding:"4px 10px",cursor:"pointer",fontSize:11,fontWeight:600}}>
                      {r}
                    </button>
                  ))}
                </div>
            ...
            {step===3&&(
              <div>
                ...
                />
                {label("DURATION")}
                <div style={{padding:"12px 14px",border:"1px solid "+GOLDDIM,background:"#1C1F25",marginBottom:8}}>
                  <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:8}}>
                    <span style={{color:GOLDDIM,fontSize:11,fontWeight:600,letterSpacing:0.2}}>Film length</span>
                    <span style={{color:GOLD,fontSize:13,fontWeight:600}}>{config.durationMin>0?(config.durationMin+" min"):"AUTO â match song"}</span>
                  </div>
                  ...
                  <div style={{color:config.durationMin>0?GOLD:GOLDDIM,fontSize:11,lineHeight:1.7,marginTop:8}}>
                    {config.durationMin>0
                      ? "This overrides the song length â the film will run exactly"+config.durationMin+" minute"+(config.durationMin===1?"":"s")+", looping or trimming the audio to fit."
            ...
            {step===4&&(
              <div>
                ...
                <div style={{background:"#1C1F25",border:"2px dashed "+(audioFile?GOLD:GOLDDIM),padding:"16px 12px",cursor:"pointer",marginBottom:8,transition:"border-color .2s"}}
                  onClick={()=>audioInputRef.current&&audioInputRef.current.click()}
                  onDragOver={e=>{e.preventDefault();e.currentTarget.style.borderColor=GOLD;e.currentTarget.style.background="#1C1F25";}}
                  onDragLeave={e=>{e.currentTarget.style.borderColor=audioFile?GOLD:GOLDDIM;e.currentTarget.style.background="#000";}}
                  onDrop={e=>{
                  ...
                  <div style={{color:audioFile?"#22c55e":WHITE,fontWeight:600,fontSize:12,letterSpacing:0.2,textAlign:"center"}}>
                    {audioFile?"â "+audioName:"DRAG & DROP or CLICK â MP3 / WAV / M4A"}
                  </div>
                ...
                <textarea
                  ...
                  style={{width:"100%",background:"#1C1F25",border:"1px solid "+GOLDDIM,padding:"12px",color:WHITE,fontSize:13,outline:"none",fontFamily:"'Archivo',system-ui,sans-serif",boxSizing:"border-box",height:130,resize:"vertical",lineHeight:1.8,marginBottom:10}}
                />
                ...
                {config.refMedia?(
                  <div style={{position:"relative",marginBottom:10}}>
                    ...
                    <button onClick={()=>set("refMedia",null)} style={{position:"absolute",top:4,right:4,background:"#1C1F25",border:"1px solid "+GOLDDIM,color:GOLD,padding:"1px 7px",cursor:"pointer",fontSize:10,fontWeight:600}}>â</button>
                    <div style={{color:"#22c55e",fontSize:9,fontWeight:600,letterSpacing:0.2,marginTop:3}}>Reference loaded</div>
                  </div>
                ):(
                  <div>
                    <div
                      onDragOver={e=>{e.preventDefault();e.currentTarget.style.borderColor=GOLD;e.currentTarget.style.background="#22262D";}}
                      onDragLeave={e=>{e.currentTarget.style.borderColor=GOLDDIM;e.currentTarget.style.background="#1C1F25";}}
                      onDrop={e=>{
                        e.preventDefault();
                        e.currentTarget.style.borderColor=GOLDDIM;e.currentTarget.style.background="#1C1F25";
                        const f=e.dataTransfer.files&&e.dataTransfer.files[0];
                      ...
                      style={{background:"#1C1F25",border:"2px dashed "+GOLDDIM,padding:"18px 10px",textAlign:"center",cursor:"pointer",marginBottom:6,transition:"all .2s"}}>
                      <div style={{color:GOLD,fontSize:14,fontWeight:600,letterSpacing:0.2,marginBottom:4}}>Drag & drop here</div>
                      <div style={{color:GOLDDIM,fontSize:10,letterSpacing:0.2}}>or click to browse â JPG Â· PNG Â· MP4</div>
                    </div>
                    <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:6,marginBottom:10}}>
                      <button onClick={()=>{const inp=document.createElement("input");inp.type="file";inp.accept="image/*,.jpg,.jpeg,.png,.gif,.webp,.heic,.heif";inp.onchange=e=>{const f=e.target.files&&e.target.files[0];if(f)set("refMedia",URL.createObjectURL(f));};inp.click();}}
                        style={{background:"#1C1F25",border:"2px solid "+SIGNAL,color:GOLD,padding:"10px",cursor:"pointer",fontSize:11,fontWeight:600,letterSpacing:0,fontFamily:"'Archivo',system-ui,sans-serif"}}>
                        Upload photo
                      </button>
                      <button onClick={()=>{const inp=document.createElement("input");inp.type="file";inp.accept="image/*,video/*";inp.onchange=e=>{const f=e.target.files&&e.target.files[0];if(f)set("refMedia",URL.createObjectURL(f));};inp.click();}}
                        style={{background:"#1C1F25",border:"1px solid "+GOLDDIM,color:WHITE,padding:"10px",cursor:"pointer",fontSize:11,fontWeight:600,letterSpacing:0,fontFamily:"'Archivo',system-ui,sans-serif"}}>
                        Upload file
                      </button>
                    </div>
                  </div>
                ...
                {/* Summary */}
                <div style={{background:"#1C1F25",border:"1px solid "+GOLDDIM,padding:14,marginBottom:14}}>
                ...
                <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:10}}>
                  <button onClick={()=>set("lipSync",!config.lipSync)}
                    style={{background:config.lipSync?GOLD:"#1C1F25",border:"1px solid "+(config.lipSync?"#000":GOLDDIM),color:config.lipSync?"#000":WHITE,padding:"6px 14px",cursor:"pointer",fontSize:11,fontWeight:600,letterSpacing:0.2,fontFamily:"'Archivo',system-ui,sans-serif"}}>
                ...
                <button onClick={generateVideo} disabled={generating}
                  ...
                  {generating?"â³ RENDERING... "+renderProgress+"%":"Generate music video"}
                </button>
                {generating&&(
                  <div>
                    <div style={{height:5,background:"#1C1F25",marginBottom:6}}>
                      <div style={{width:renderProgress+"%",height:"100%",background:"#1C1F25",transition:"width .3s"}}/>
                    </div>
                    <div style={{background:"#1C1F25",border:"1px solid "+GOLDDIM,padding:10,maxHeight:140,overflowY:"auto"}}>
                      {renderLog.map((l,i)=>(
                ...
                {!generating&&renderLog.length>0&&(
                  <div style={{background:"#1C1F25",border:"1px solid "+GOLDDIM,padding:10,maxHeight:120,overflowY:"auto"}}>
                    {renderLog.map((l,i)=>(
                      <div key={i} style={{color:i===renderLog.length-1?"#22c55e":DIM,fontSize:10,lineHeight:1.8}}>
                        {i===renderLog.length-1?"â¶ ":"  "}{l}
                      </div>
          ...
          {videoUrl&&(
            <div style={{display:"flex",flexDirection:"column",background:"#1C1F25",overflow:"hidden"}}>
              {/* Video player */}
              <div style={{position:"relative",background:"#1C1F25"}}>
                <canvas ref={canvasRef} style={{position:"fixed",right:8,bottom:8,width:160,height:90,opacity:1,pointerEvents:"none",zIndex:9999,border:"1px solid #e8c96d",background:"#1C1F25"}}/>
                <video ref={videoRef} src={videoUrl} playsInline
                  style={{width:"100%",aspectRatio:"16/9",display:"block",background:"#1C1F25"}}
                ...
                <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:5,marginBottom:12}}>
                  {SOCIAL.map(([name,color,url])=>(
                    <button key={name} onClick={()=>window.open(url,"_blank")}
                      style={{background:"#1C1F25",border:"1px solid "+color+"33",color:color,padding:"7px 4px",cursor:"pointer",fontSize:10,fontWeight:600,letterSpacing:0,fontFamily:"'Archivo',system-ui,sans-serif"}}
          ...
          {!videoUrl&&<canvas ref={canvasRef} style={{position:"fixed",right:8,bottom:8,width:160,height:90,opacity:1,pointerEvents:"none",zIndex:9999,border:"1px solid #e8c96d",background:"#1C1F25"}}/>}
        </div>
...
const VOICE_CHARACTERS = [
  ...
  {id:"james",name:"James",emoji:"",gender:"Male",age:"Adult",origin:"British",region:"London",style:"Sarcastic Â· Deadpan Â· Witty",pitch:0.86,rate:0.62,desc:"Dry British wit. Devastating things said with complete calm."},
  {id:"aurora",name:"Aurora",emoji:"",gender:"Female",age:"Adult",origin:"British",region:"London",style:"Warm Â· Documentary Â· Authoritative",pitch:1.08,rate:0.80,desc:"Calm authority. The voice you trust completely."},
  {id:"edward",name:"Edward",emoji:"",gender:"Male",age:"Adult",origin:"British",region:"London",style:"Theatrical Â· Grand Â· Classical",pitch:0.85,rate:0.75,desc:"Shakespearean gravitas. Every sentence carved in stone."},
  {id:"cecily",name:"Cecily",emoji:"",gender:"Female",age:"Adult",origin:"British",region:"London",style:"Crisp Â· Intelligent Â· Sardonic",pitch:1.12,rate:0.85,desc:"Sharp as a tack. Mildly disappointed by most things."},
  {id:"nana",name:"Nana",emoji:"",gender:"Female",age:"Elderly",origin:"British",region:"Yorkshire",style:"Gentle Â· Wise Â· Warm",pitch:1.02,rate:0.70,desc:"Warm elderly wisdom. Has seen everything twice."},
  {id:"colonel",name:"Colonel",emoji:"",gender:"Male",age:"Elderly",origin:"British",region:"London",style:"Commanding Â· Dignified Â· Veteran",pitch:0.80,rate:0.74,desc:"Authority earned through decades of experience."},
  {id:"pippa",name:"Pippa",emoji:"",gender:"Female",age:"Teen",origin:"British",region:"London",style:"Bright Â· Cheerful Â· Young",pitch:1.25,rate:0.95,desc:"Fresh and warm. Natural young British energy."},
  {id:"archie",name:"Archie",emoji:"â½",gender:"Male",age:"Teen",origin:"British",region:"Manchester",style:"Casual Â· Friendly Â· Teen",pitch:1.05,rate:0.98,desc:"Relaxed and genuine. Sounds like a real teenager."},
  {id:"ewan",name:"Ewan",emoji:"",gender:"Male",age:"Adult",origin:"Scottish",region:"Edinburgh",style:"Warm Â· Rugged Â· Sincere",pitch:0.92,rate:0.82,desc:"Deep warm Scottish sincerity."},
  {id:"fiona",name:"Fiona",emoji:"",gender:"Female",age:"Adult",origin:"Scottish",region:"Glasgow",style:"Lilting Â· Warm Â· Storyteller",pitch:1.10,rate:0.84,desc:"Beautiful Scottish lilt."},
  {id:"paddy",name:"Paddy",emoji:"â",gender:"Male",age:"Adult",origin:"Irish",region:"Dublin",style:"Charming Â· Witty Â· Warm",pitch:0.95,rate:0.88,desc:"Easy Irish charm."},
  {id:"siobhan",name:"Siobhan",emoji:"",gender:"Female",age:"Adult",origin:"Irish",region:"Cork",style:"Gentle Â· Musical Â· Emotional",pitch:1.15,rate:0.82,desc:"Soft Irish voice with real emotional depth."},
  {id:"dafydd",name:"Dafydd",emoji:"",gender:"Male",age:"Adult",origin:"Welsh",region:"Cardiff",style:"Musical Â· Passionate Â· Rich",pitch:0.90,rate:0.80,desc:"Rich Welsh musicality."},
  {id:"marcus",name:"Marcus",emoji:"",gender:"Male",age:"Adult",origin:"American",region:"New York",style:"Deep Â· Cinematic Â· Commanding",pitch:0.72,rate:0.74,desc:"Big voice. When Marcus speaks people stop."},
  {id:"river",name:"River",emoji:"",gender:"Male",age:"Adult",origin:"American",region:"Tennessee",style:"Warm Â· Intimate Â· Storyteller",pitch:0.98,rate:0.76,desc:"Unhurried Southern charm."},
  {id:"dakota",name:"Dakota",emoji:"",gender:"Female",age:"Adult",origin:"American",region:"Chicago",style:"Bold Â· Direct Â· Confident",pitch:1.05,rate:0.92,desc:"No filler. No hesitation."},
  {id:"wade",name:"Wade",emoji:"",gender:"Male",age:"Adult",origin:"American",region:"Texas",style:"Laid Back Â· Humorous Â· Folksy",pitch:0.94,rate:0.85,desc:"Easy going Southern humour."},
  {id:"brooklyn",name:"Brooklyn",emoji:"",gender:"Female",age:"Adult",origin:"American",region:"New York",style:"Fast Â· Sharp Â· City Energy",pitch:1.18,rate:1.10,desc:"Fast New York energy."},
  {id:"savannah",name:"Savannah",emoji:"",gender:"Female",age:"Adult",origin:"American",region:"Georgia",style:"Sweet Â· Gracious Â· Warm",pitch:1.20,rate:0.84,desc:"Warm Southern grace."},
  {id:"madison",name:"Madison",emoji:"",gender:"Female",age:"Teen",origin:"American",region:"California",style:"Upbeat Â· Social Â· Natural",pitch:1.30,rate:1.08,desc:"Real American teenage energy."},
  {id:"tyler",name:"Tyler",emoji:"",gender:"Male",age:"Teen",origin:"American",region:"Ohio",style:"Casual Â· Relatable Â· Teen",pitch:1.08,rate:1.00,desc:"Natural and unforced."},
  {id:"rosie",name:"Rosie",emoji:"",gender:"Female",age:"Child",origin:"American",region:"Florida",style:"Sweet Â· Innocent Â· Child",pitch:1.45,rate:0.88,desc:"Young warm and sweet."},
  {id:"cooper",name:"Cooper",emoji:"",gender:"Male",age:"Child",origin:"American",region:"Colorado",style:"Bright Â· Curious Â· Child",pitch:1.40,rate:0.90,desc:"Curious about everything."},
  {id:"grandma",name:"Grandma",emoji:"",gender:"Female",age:"Elderly",origin:"American",region:"Virginia",style:"Warm Â· Loving Â· Elderly",pitch:1.00,rate:0.72,desc:"Full of love and life experience."},
  {id:"frank",name:"Frank",emoji:"",gender:"Male",age:"Elderly",origin:"American",region:"New Jersey",style:"Gruff Â· Honest Â· Elder",pitch:0.78,rate:0.76,desc:"Says it straight."},
  {id:"sophia",name:"Sophia",emoji:"â",gender:"Female",age:"Adult",origin:"Australian",region:"Sydney",style:"Upbeat Â· Bright Â· Energetic",pitch:1.35,rate:1.12,desc:"Forward energy."},
  {id:"finn",name:"Finn",emoji:"",gender:"Male",age:"Adult",origin:"Australian",region:"Melbourne",style:"Casual Â· Confident Â· Outdoorsy",pitch:0.95,rate:0.95,desc:"Relaxed Australian confidence."},
  {id:"aroha",name:"Aroha",emoji:"",gender:"Female",age:"Adult",origin:"New Zealand",region:"Auckland",style:"Warm Â· Grounded Â· Sincere",pitch:1.10,rate:0.86,desc:"Natural sincerity."},
  {id:"amara",name:"Amara",emoji:"",gender:"Female",age:"Adult",origin:"South African",region:"Cape Town",style:"Rich Â· Warm Â· Powerful",pitch:1.05,rate:0.84,desc:"Quiet power."},
  {id:"kofi",name:"Kofi",emoji:"",gender:"Male",age:"Adult",origin:"West African",region:"Ghana",style:"Deep Â· Rhythmic Â· Storyteller",pitch:0.82,rate:0.78,desc:"Every sentence has music in it."},
  {id:"priya",name:"Priya",emoji:"",gender:"Female",age:"Adult",origin:"Indian",region:"Mumbai",style:"Precise Â· Warm Â· Intelligent",pitch:1.15,rate:0.90,desc:"Warm and intelligent."},
  {id:"arjun",name:"Arjun",emoji:"",gender:"Male",age:"Adult",origin:"Indian",region:"Delhi",style:"Authoritative Â· Clear Â· Measured",pitch:0.88,rate:0.85,desc:"Sounds like someone who knows exactly what they are talking about."},
  {id:"valentina",name:"Valentina",emoji:"",gender:"Female",age:"Adult",origin:"Spanish",region:"Madrid",style:"Passionate Â· Warm Â· Expressive",pitch:1.18,rate:0.92,desc:"Everything sounds felt."},
  {id:"pierre",name:"Pierre",emoji:"",gender:"Male",age:"Adult",origin:"French",region:"Paris",style:"Suave Â· Dry Â· Cultured",pitch:0.90,rate:0.84,desc:"Makes things sound interesting."},
  {id:"ingrid",name:"Ingrid",emoji:"â",gender:"Female",age:"Adult",origin:"Scandinavian",region:"Stockholm",style:"Clean Â· Cool Â· Direct",pitch:1.08,rate:0.88,desc:"No excess words."},
  {id:"yemi",name:"Yemi",emoji:"",gender:"Female",age:"Adult",origin:"Nigerian",region:"Lagos",style:"Bold Â· Joyful Â· Energetic",pitch:1.25,rate:1.00,desc:"Life-affirming."},
  {id:"magnus",name:"Magnus",emoji:"",gender:"Male",age:"Elderly",origin:"Fantasy",region:"Ancient",style:"Ancient Â· Wise Â· Epic",pitch:0.75,rate:0.70,desc:"Seen civilisations rise and fall."},
  {id:"nova",name:"Nova",emoji:"",gender:"Female",age:"Adult",origin:"Neutral",region:"AI",style:"Clean Â· Precise Â· Neutral",pitch:1.12,rate:0.95,desc:"No accent. No emotion. No opinion."},
  {id:"hunter",name:"Hunter",emoji:"",gender:"Male",age:"Adult",origin:"American",region:"Hollywood",style:"Trailer Â· Epic Â· Explosive",pitch:0.70,rate:0.80,desc:"Full movie trailer energy."},
  {id:"luna",name:"Luna",emoji:"",gender:"Female",age:"Adult",origin:"Neutral",region:"ASMR",style:"Whisper Â· ASMR Â· Intimate",pitch:1.20,rate:0.65,desc:"Soft whisper. Complete calm."},
  {id:"professor",name:"Professor",emoji:"",gender:"Male",age:"Elderly",origin:"British",region:"Oxford",style:"Academic Â· Thoughtful Â· Measured",pitch:0.88,rate:0.78,desc:"Distinguished. Precise."},
  {id:"hope",name:"Hope",emoji:"",gender:"Female",age:"Adult",origin:"American",region:"Heartfelt",style:"Tender Â· Gentle Â· Loving",pitch:1.15,rate:0.78,desc:"Pure tenderness."},
  {id:"storm",name:"Storm",emoji:"â",gender:"Male",age:"Adult",origin:"American",region:"Intense",style:"Intense Â· Angry Â· Powerful",pitch:0.82,rate:1.00,desc:"Raw intensity."},
  {id:"joy",name:"Joy",emoji:"",gender:"Female",age:"Adult",origin:"American",region:"Uplifting",style:"Excited Â· Joyful Â· Celebratory",pitch:1.40,rate:1.15,desc:"Pure infectious joy."},
  {id:"sage",name:"Sage",emoji:"",gender:"Male",age:"Adult",origin:"Neutral",region:"Mindful",style:"Peaceful Â· Mindful Â· Grounded",pitch:0.95,rate:0.72,desc:"Deep calm."},
  {id:"faith",name:"Faith",emoji:"",gender:"Female",age:"Adult",origin:"American",region:"Gospel",style:"Inspirational Â· Gospel Â· Uplifting",pitch:1.18,rate:0.88,desc:"Gospel soul."},
  {id:"rebel",name:"Rebel",emoji:"â",gender:"Female",age:"Teen",origin:"American",region:"Activist",style:"Fierce Â· Defiant Â· Young",pitch:1.22,rate:1.05,desc:"Will not back down."},
  {id:"blaze",name:"Blaze",emoji:"",gender:"Female",age:"Adult",origin:"American",region:"Cinematic",style:"Warm Â· Confident Â· Cinematic",pitch:1.02,rate:0.95,desc:"Warm cinematic narrator."},
  {id:"remy",name:"Remy",emoji:"",gender:"Male",age:"Adult",origin:"French",region:"Lyon",style:"Smooth Â· Romantic Â· Intimate",pitch:0.92,rate:0.80,desc:"Everything sounds like poetry."},
  {id:"zhara",name:"Zhara",emoji:"",gender:"Female",age:"Adult",origin:"Middle Eastern",region:"Dubai",style:"Elegant Â· Warm Â· Sophisticated",pitch:1.10,rate:0.85,desc:"Graceful and precise."},
  {id:"kai",name:"Kai",emoji:"",gender:"Male",age:"Adult",origin:"Hawaiian",region:"Honolulu",style:"Relaxed Â· Warm Â· Soulful",pitch:0.96,rate:0.82,desc:"Unhurried ocean warmth."},
  {id:"sienna",name:"Sienna",emoji:"",gender:"Female",age:"Adult",origin:"American",region:"New Orleans",style:"Soulful Â· Blues Â· Deep",pitch:1.05,rate:0.78,desc:"Every word feels lived-in."},
  {id:"atlas",name:"Atlas",emoji:"",gender:"Male",age:"Adult",origin:"Neutral",region:"Epic",style:"Cinematic Â· Epic Â· Booming",pitch:0.68,rate:0.76,desc:"The voice of a thousand documentaries."},
  {id:"echo",name:"Echo",emoji:"",gender:"Female",age:"Adult",origin:"Neutral",region:"Ethereal",style:"Ethereal Â· Dreamy Â· Otherworldly",pitch:1.22,rate:0.72,desc:"Sounds like it came from somewhere else."},
];
...
function P6Voice({ onSave, setMediaLib }) {
  ...
  const addMyVoice=async(file)=>{
    ...
    const nv={id,name:(file.name||"My Voice").replace(/\.[^.]+$/,""),url,dbId:id,origin:"My Upload",gender:"â",age:"â",emoji:"ð",style:"Your recorded voice",desc:"Your own voice â plays back exactly as recorded."};
  ...
  const inp={width:"100%",background:"#1C1F25",border:"1px solid "+GOLDDIM,padding:"12px 14px",color:WHITE,fontSize:14,outline:"none",boxSizing:"border-box",fontFamily:"'Archivo',system-ui,sans-serif",lineHeight:1.9};
  ...
  return(
    <div style={{...Sp}}>
      ...
      <div style={{padding:"12px 18px",borderBottom:"1px solid "+GOLDDIM+"",display:"flex",alignItems:"center",justifyContent:"space-between",flexWrap:"wrap",gap:10}}>
        <div><div style={{fontSize:11,color:GOLD,letterSpacing:0.4,fontWeight:500}}>AI workstation 02 â cinema voice engine</div><h1 style={{...H1,fontSize:24,margin:0}}>Text to lifelike speech</h1></div>
      ...
      <div style={{display:"grid",gridTemplateColumns:"290px 1fr",minHeight:"calc(100vh - 120px)"}}>
        <div style={{borderRight:"1px solid "+GOLDDIM+"",background:"#030303",display:"flex",flexDirection:"column"}}>
          <div style={{padding:"10px 10px 6px"}}>
            <div style={{color:GOLD,fontSize:11,letterSpacing:0.2,fontWeight:600,marginBottom:8}}>Voice library â {filtered.length} / {VOICE_CHARACTERS.length}</div>
            <div style={{marginBottom:5}}><div style={{color:GOLDDIM,fontSize:9,letterSpacing:0.2,marginBottom:3}}>Gender</div><div style={{display:"flex",gap:4}}>{GENDERS.map(g=><button key={g} onClick={()=>setFilterGender(g)} style={{flex:1,background:filterGender===g?GOLD:"#1C1F25",border:"1px solid "+(filterGender===g?"#000":GOLDDIM),color:filterGender===g?"#000":WHITE,padding:"3px 0",cursor:"pointer",fontSize:10,fontWeight:600}}>{g}</button>)}</div></div>
            <div style={{marginBottom:5}}><div style={{color:GOLDDIM,fontSize:9,letterSpacing:0.2,marginBottom:3}}>Age</div><div style={{display:"flex",flexWrap:"wrap",gap:3}}>{AGES.map(a=><button key={a} onClick={()=>setFilterAge(a)} style={{background:filterAge===a?GOLD:"#1C1F25",border:"1px solid "+(filterAge===a?"#000":GOLDDIM),color:filterAge===a?"#000":WHITE,padding:"2px 8px",cursor:"pointer",fontSize:9,fontWeight:600}}>{a}</button>)}</div></div>
            <div style={{marginBottom:6}}><div style={{color:GOLDDIM,fontSize:9,letterSpacing:0.2,marginBottom:3}}>Origin</div><select value={filterOrigin} onChange={e=>setFilterOrigin(e.target.value)} style={{width:"100%",background:"#1C1F25",border:"1px solid "+GOLDDIM,color:WHITE,padding:"4px 8px",fontSize:11,outline:"none"}}>{ORIGINS.map(o=><option key={o} value={o}>{o}</option>)}</select></div>
          ...
          <div style={{flex:1,overflowY:"auto",padding:"6px 6px 80px"}}>
            ...
            {recordingMine?(
              <div style={{display:"flex",alignItems:"center",gap:8,background:"#1a0000",border:"2px solid #ef4444",padding:"9px 12px",marginBottom:6}}>
                <div style={{width:10,height:10,borderRadius:"50%",background:"#ef4444",boxShadow:"0 0 8px #ef4444"}}/>
                <span style={{color:"#ef4444",fontWeight:600,fontSize:11,letterSpacing:0.2,flex:1}}>Recording â {String(Math.floor(recTime/60)).padStart(2,"0")}:{String(recTime%60).padStart(2,"0")}</span>
              ...
              <button onClick={startMyRecording} style={{width:"100%",background:"#1C1F25",border:"none",color:"#fff",padding:"10px",cursor:"pointer",fontSize:11,fontWeight:600,letterSpacing:0.2,fontFamily:"'Archivo',system-ui,sans-serif",marginBottom:6}}>Record my voice now</button>
            )}
            <button onClick={()=>myVoiceInputRef.current&&myVoiceInputRef.current.click()} style={{width:"100%",background:"#1C1F25",border:"2px solid "+SIGNAL,color:GOLD,padding:"10px",cursor:"pointer",fontSize:11,fontWeight:600,letterSpacing:0.2,fontFamily:"'Archivo',system-ui,sans-serif",marginBottom:6}}>ï¼ add your own voice (file)</button>
            {(<>
              <div style={{color:GOLDDIM,fontSize:10,lineHeight:1.5,marginBottom:6,letterSpacing:0}}>Record just the FIRST PARAGRAPH in your own voice â the engine clones your voice and reads the rest of the narration to the end in YOUR voice, wired into the generator and render.</div>
              ...
              {(<>
                ...
                <button onClick={engineCompleteNarration} disabled={narrBusy} style={{width:"100%",background:narrBusy?"#22262D":"#1C1F25",border:"2px solid "+SIGNAL,color:GOLD,padding:"11px",cursor:narrBusy?"wait":"pointer",fontSize:11,fontWeight:600,letterSpacing:0.2,fontFamily:"'Archivo',system-ui,sans-serif",marginBottom:6}}>{narrBusy?"â³ CLONING & COMPLETINGâ¦":"Use engine to complete full narration"}</button>
              </>)}
            </>)}
            {myVoices.map(v=>(
              <div key={v.id} onClick={()=>setSelVoice(v.id)} style={{padding:"10px 12px",marginBottom:4,background:selVoice===v.id?"#1C1F25":"#000",border:"2px solid "+(selVoice===v.id?GOLD:GOLDDIM),cursor:"pointer"}}>
                <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                  <div style={{display:"flex",alignItems:"center",gap:6}}>
                    ...
                    <div><div style={{color:selVoice===v.id?GOLD:WHITE,fontSize:13,fontWeight:600}}>{v.name}{v.clonedVoiceId&&<span style={{color:GOLD,fontSize:10,marginLeft:6}}>Cloned</span>}</div><div style={{color:GOLDDIM,fontSize:10}}>{v.clonedVoiceId?"Your cloned voice â engine narrates in your voice":"Your uploaded voice"}</div></div>
                  </div>
                  <div style={{display:"flex",gap:4,flexShrink:0}}>
                    {v.url&&<button onClick={e=>{e.stopPropagation();const a=new Audio(v.url);a.play().catch(()=>{});}} style={{background:GOLDDIM,border:"none",color:"#000",padding:"3px 8px",cursor:"pointer",fontSize:9,fontWeight:600}}>â¶</button>}
                    <button onClick={e=>{e.stopPropagation();delMyVoice(v.id);}} style={{background:"#1C1F25",border:"1px solid "+GOLDDIM,color:GOLD,padding:"3px 8px",cursor:"pointer",fontSize:9,fontWeight:600}}>â</button>
                  </div>
                </div>
                {selVoice===v.id&&<div style={{color:GOLD,fontSize:9,letterSpacing:0.2,marginTop:4,fontWeight:600}}>{cloning?"Cloning your voiceâ¦":"Selected"}</div>}
              </div>
            ))}
            {filtered.map(v=>(
              <div key={v.id} onClick={()=>setSelVoice(v.id)} style={{padding:"10px 12px",marginBottom:4,background:selVoice===v.id?"#1C1F25":"#000",border:"2px solid "+(selVoice===v.id?GOLD:GOLDDIM),cursor:"pointer"}}>
                <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:4}}>
                  <div style={{display:"flex",alignItems:"center",gap:6}}>
                    ...
                    <div><div style={{color:selVoice===v.id?GOLD:WHITE,fontSize:13,fontWeight:600}}>{v.name}</div><div style={{color:GOLDDIM,fontSize:10}}>{v.origin} Â· {v.gender} Â· {v.age}</div></div>
                  </div>
        ...
        <div style={{display:"flex",flexDirection:"column",background:"#030303",overflowY:"auto",padding:20}}>
          <div style={{background:"#1C1F25",border:"1px solid "+GOLDDIM,padding:"10px 14px",marginBottom:14}}>
            <div style={{color:WHITE,fontSize:13,fontWeight:600}}>{selected.name} {selected.emoji} Â· {selected.origin} Â· {selected.gender}</div>
          ...
          <div style={{background:"#1C1F25",border:"1px solid "+GOLDDIM,padding:"12px 14px",marginBottom:14}}>
            ...
            <div style={{marginBottom:10}}><div style={{color:GOLDDIM,fontSize:10,fontWeight:600,letterSpacing:0.2,marginBottom:4}}>Mood</div>
              <select value={mood} onChange={e=>setMood(e.target.value)} style={{width:"100%",background:"#1C1F25",border:"1px solid "+GOLDDIM,color:WHITE,padding:"8px 12px",fontSize:13,fontFamily:"'Archivo',system-ui,sans-serif",outline:"none"}}>
                {["Neutral","Happy","Sad","Angry","Excited","Calm","Dramatic","Mysterious","Romantic","Sarcastic","Melancholic","Authoritative","Warm"].map(m=><option key={m} value={m} style={{background:"#1C1F25"}}>{m}</option>)}
              </select>
            </div>
          ...
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8,marginBottom:14}}>
            <div style={{background:"#1C1F25",border:"1px solid "+GOLDDIM,padding:"12px 14px"}}>
          ...
          {savedToLib&&<div style={{background:"#061406",border:"1px solid #22c55e",padding:"10px 14px",textAlign:"center",marginBottom:8}}><span style={{color:"#22c55e",fontWeight:600,fontSize:12,letterSpacing:0.2}}>Narration saved to media library â auto-added to timeline</span></div>}
        </div>
      </div>
    </div>
  );
...
function MSUserCounter(){
  ...
  return (
    <div style={{display:"flex",justifyContent:"center",marginBottom:24}}>
      <div style={{background:"#14161A",border:"2px solid "+SIGNAL,padding:"14px 40px",textAlign:"center",boxShadow:"none",position:"relative"}}>
        <div style={{display:"flex",alignItems:"center",justifyContent:"center",gap:6,marginBottom:6}}>
          ...
          <div style={{color:"#22c55e",fontSize:10,letterSpacing:0.4,fontWeight:600}}>Live Â· user count</div>
        </div>
        ...
        <div style={{color:GOLDDIM,fontSize:9,letterSpacing:0.2,marginTop:8}}>launched june 1st 2026 Â· updates automatically</div>
...
function P8VideoGenerator({ onSave, user, filmDuration, setFilmDuration }) {
  ...
  // MAKE MY MOVIE â one-shot: drop everything in, set the length, press go.
  ...
  // is stitched into one film â all on this page. Nothing navigates away.
  // ââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââ
  ...
  // ââ FREE CANVAS FALLBACK ââââââââââââââââââââââââââââââââââââââââââââââââ
      ...
      await new Promise(resolve=>{
        ...
        const tick=()=>{
          ...
          try{
            ...
            else { const g=ctx.createLinearGradient(0,0,0,H); g.addColorStop(0,"#1a1207"); g.addColorStop(1,"#14161A"); ctx.fillStyle=g; ctx.fillRect(0,0,W,H); }
          }catch(e){ ctx.fillStyle="#14161A"; ctx.fillRect(0,0,W,H); }
  ...
  const makeMyMovie=async()=>{
    ...
    // 1 â Write the full scene list to length
    setMmmStage("Reading what you gave me and writing the scene listâ¦");
    setMmmPct(4);
    let sceneList=[];
    try{
      ...
      const ask=
        ...
        "RULES:\n"+
        "1. FIRST expand what the user actually gave you into detailed shots â their material always leads.\n"+
        ...
        "MATERIAL:\n"+(source||"(no script â build the film from the uploaded images and the genre)")+"\n\n"+
    ...
    if(!sceneList.length){ setMmmError("Couldn't build a scene list â try adding a bit more detail."); setMmmBusy(false); return; }
    setMmmScenes(sceneList.map(s=>({text:s,status:"waiting"})));
    setMmmStage("Scene list ready â "+sceneList.length+" scenes. Renderingâ¦");
    setMmmPct(10);

    // 2 â Render every scene through the Cinema Engine and save each one
    ...
    for(let i=0;i<sceneList.length;i++){
      ...
      setMmmStage("Rendering scene "+(i+1)+" of "+sceneList.length+"â¦");
      let url="";
      let usedFallback=false;
      try{
        url=await engineRender(sceneList[i],{duration:perSceneSec,image:i===0?firstImg:"",aspect_ratio:"16:9"});
      }catch(e){ url=""; }
      // Engine gave us nothing â draw it here instead, free, on this device.
      if(!url){
        setMmmStage("Scene "+(i+1)+" â drawing on this device (no engine credit needed)â¦");
    ...
    if(!clipUrls.length){ setMmmError("No footage came back â the engine had no credit and the on-device renderer couldn't draw either. Check you're in Chrome with this tab in front, then run it again."); setMmmBusy(false); return; }

    // 3 â Stitch into one film (concatenated playback) â stays on this page
    setMmmStage("Stitching your film togetherâ¦");
    setMmmPct(94);
    try{
  ...
  // Sequential player: when one scene ends, play the next â feels like one film.
  ...
  // ---- REAL STITCH: weld every clip in the playlist into ONE continuous video file ----
  // Plays each clip through a single canvas and records the whole run into one blob.
  // Real-time: a 10-min movie takes ~10 min to stitch. Long films strain the tab.
  const [mmmStitchBusy,setMmmStitchBusy]=useState(false);
  const [mmmStitchPct,setMmmStitchPct]=useState(0);
  const [mmmStitchStage,setMmmStitchStage]=useState("");
  const mmmStitchFilm=async()=>{
    let list=[]; try{list=JSON.parse(localStorage.getItem("ms_mmm_playlist")||"[]");}catch(e){}
    if(!list.length){ setMmmError&&setMmmError("No movie to stitch yet - make your movie first."); return ""; }
    if(list.length===1){
      try{
        const b=await (await fetch(list[0])).blob();
        const a=document.createElement("a"); a.href=URL.createObjectURL(b);
        a.download="MakeMyMovie.webm"; document.body.appendChild(a); a.click(); a.remove();
      }catch(e){}
      return "";
    }
    setMmmStitchBusy(true); setMmmStitchPct(0); setMmmStitchStage("Preparing to stitch "+list.length+" scenes...");
    try{
      const W=1280,H=720,fps=24;
      const cv=document.createElement("canvas"); cv.width=W; cv.height=H;
      const ctx=cv.getContext("2d");
      ...
      try{ audioCtx=new (window.AudioContext||window.webkitAudioContext)(); dest=audioCtx.createMediaStreamDestination(); }catch(e){}

      const canvasStream=cv.captureStream(fps);
      let outStream=canvasStream;
      if(dest&&dest.stream){
        try{ outStream=new MediaStream([...canvasStream.getVideoTracks(),...dest.stream.getAudioTracks()]); }catch(e){ outStream=canvasStream; }
      }

      let mime="video/webm";
      try{
        const opts=["video/webm;codecs=vp9,opus","video/webm;codecs=vp8,opus","video/webm;codecs=vp9","video/webm;codecs=vp8","video/webm","video/mp4"];
        for(const m of opts){ if(window.MediaRecorder&&MediaRecorder.isTypeSupported(m)){ mime=m; break; } }
      }catch(e){}
      const rec=new MediaRecorder(outStream,{mimeType:mime,videoBitsPerSecond:5000000});
      const chunks=[];
      rec.ondataavailable=ev=>{ if(ev.data&&ev.data.size)chunks.push(ev.data); };
      const recStopped=new Promise(r=>{ rec.onstop=()=>r(null); });
      rec.start(400);

      let stop=false; let curVideo=null;
      const pump=()=>{ if(stop)return; try{ if(curVideo&&!curVideo.paused&&!curVideo.ended&&curVideo.readyState>=2){ ctx.drawImage(curVideo,0,0,W,H);} }catch(e){} requestAnimationFrame(pump); };
      requestAnimationFrame(pump);

      for(let i=0;i<list.length;i++){
        setMmmStitchStage("Stitching scene "+(i+1)+" of "+list.length+"...");
        setMmmStitchPct(Math.round((i/list.length)*100));
        await new Promise((resolve)=>{
          const v=document.createElement("video");
          v.src=list[i]; v.crossOrigin="anonymous"; v.muted=false; v.playsInline=true; v.preload="auto";
          curVideo=v;
          let piped=false;
          const wire=()=>{ if(piped||!audioCtx||!dest)return; try{ const s=audioCtx.createMediaElementSource(v); s.connect(dest); piped=true; }catch(e){} };
          v.onloadedmetadata=()=>{ wire(); };
          v.onended=()=>{ resolve(null); };
          v.onerror=()=>{ resolve(null); };
          const go=()=>{ try{ if(audioCtx&&audioCtx.state==="suspended")audioCtx.resume(); }catch(e){} wire(); v.play().catch(()=>{ resolve(null); }); };
          if(v.readyState>=2) go(); else v.oncanplay=go;
          setTimeout(()=>{ try{ if(!v.ended){ v.pause(); resolve(null);} }catch(e){ resolve(null);} },15*60*1000);
        });
      }

      stop=true;
      setMmmStitchStage("Finalizing film...");
      setMmmStitchPct(100);
      try{ rec.stop(); }catch(e){}
      await Promise.race([recStopped,new Promise(r=>setTimeout(()=>r(null),8000))]);
      try{ canvasStream.getTracks().forEach(t=>t.stop()); }catch(e){}
      try{ if(audioCtx)audioCtx.close(); }catch(e){}

      if(!chunks.length){ setMmmError&&setMmmError("Stitch produced no data. Try fewer / shorter scenes."); setMmmStitchBusy(false); return ""; }
      const blob=new Blob(chunks,{type:mime.split(";")[0]});
      const url=URL.createObjectURL(blob);
      setMmmFilmUrl&&setMmmFilmUrl(url);
      const a=document.createElement("a"); a.href=url;
      a.download="MakeMyMovie_FULL.webm"; document.body.appendChild(a); a.click(); a.remove();
      setMmmStitchBusy(false); setMmmStitchStage("");
      return url;
    }catch(e){
      setMmmError&&setMmmError("Stitch failed: "+(e&&e.message?e.message:"unknown"));
      setMmmStitchBusy(false);
      return "";
    }
  };
...
Write the drawFrame body now.`}]
      ...
      try{
        ...
        if(track){
          ...
          if(genStereo && musicCtx.createStereoPanner){
            addLog("Stereo sound ON â widening the music bed");
          ...
          addLog("Background music ready â mixing into film");
        }
    ...
    await new Promise(resolve=>{
      ...
      const tick=()=>{
        ...
        try{drawFrame(ctx,1280,720,t,sec,loadedRefImages);}catch(e){ctx.fillStyle="#14161A";ctx.fillRect(0,0,1280,720);}
  ...
  return (
    <div style={{minHeight:"100vh",background:"#1C1F25",color:WHITE,fontFamily:"'Archivo',system-ui,sans-serif",paddingBottom:160}}>
      {gapPrompt&&(
        <div style={{position:"fixed",inset:0,zIndex:2000,background:"rgba(0,0,0,0.94)",display:"flex",alignItems:"center",justifyContent:"center",padding:20}}>
          <div style={{width:"min(460px,95vw)",background:"#14161A",border:"2px solid "+SIGNAL,padding:"24px 22px"}}>
            ...
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}>
              <button disabled={gapBusy} onClick={()=>{
                ...
                style={{background:"#1C1F25",border:"none",color:"#000",padding:"14px",fontSize:13,fontWeight:600,letterSpacing:0.2,cursor:"pointer",fontFamily:"'Archivo',system-ui,sans-serif"}}>
                Yes, fill it
              </button>
      ...
      <div style={{padding:"12px 20px",borderBottom:"1px solid "+GOLDDIM+"",display:"flex",alignItems:"center",justifyContent:"space-between",flexWrap:"wrap",gap:10}}>
        <div>
          <div style={{fontSize:11,color:GOLD,letterSpacing:0.4,fontWeight:500}}>MANDASTRONG ENGINE v2 Â· CINEMA-GRADE RENDERER</div>
        ...
        <div style={{color:GOLD,fontSize:11,fontWeight:500,letterSpacing:0.2}}>Mandastrong engine Â· any prompt Â· any subject</div>
      </div>
      {mmmStudio&&(
        <div style={{position:"fixed",inset:0,zIndex:900,background:"#1C1F25",overflowY:"auto",padding:"18px 16px 60px"}}>
          <div style={{maxWidth:760,margin:"0 auto"}}>
            ...
            <textarea value={mmmText} onChange={e=>setMmmText(e.target.value)} rows={5}
              ...
              style={{width:"100%",boxSizing:"border-box",background:"#1C1F25",border:"1px solid "+GOLDDIM,borderRadius:6,color:"#fff",padding:11,fontSize:13,fontFamily:"'Archivo',system-ui,sans-serif",resize:"vertical",marginBottom:10}}/>
                ...
                {mmmImages.map((im,i)=>(
                  <div key={i} style={{position:"relative"}}>
                    ...
                    <button onClick={()=>setMmmImages(p=>p.filter((_,x)=>x!==i))}
                      style={{position:"absolute",top:-6,right:-6,width:18,height:18,borderRadius:9,background:"#1C1F25",color:GOLD,border:"1px solid "+GOLDDIM,fontSize:11,cursor:"pointer",lineHeight:"16px",padding:0}}>x</button>
                  </div>
                ))}
              </div>
            )}
            ...
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:12}}>
              <div>
                ...
                <select value={mmmStyle} onChange={e=>setMmmStyle(e.target.value)}
                  style={{width:"100%",background:"#1C1F25",border:"1px solid "+GOLDDIM,color:GOLD,padding:"9px 12px",fontSize:12,fontFamily:"'Archivo',system-ui,sans-serif",cursor:"pointer"}}>
                  {RENDER_STYLES.map(s=><option key={s.id} value={s.id} style={{background:"#1C1F25"}}>{s.label}</option>)}
                </select>
              </div>
              <div>
                ...
                <select value={mmmGenre} onChange={e=>setMmmGenre(e.target.value)}
                  style={{width:"100%",background:"#1C1F25",border:"1px solid "+GOLDDIM,color:mmmGenre?GOLD:GOLDDIM,padding:"9px 12px",fontSize:12,fontFamily:"'Archivo',system-ui,sans-serif",cursor:"pointer"}}>
                  {FILM_GENRES.map(g=><option key={g.id} value={g.id} style={{background:"#1C1F25"}}>{g.label}</option>)}
                </select>
              </div>
              <div>
                ...
                <select value={mmmGrade} onChange={e=>setMmmGrade(e.target.value)}
                  style={{width:"100%",background:"#1C1F25",border:"1px solid "+GOLDDIM,color:GOLD,padding:"9px 12px",fontSize:12,fontFamily:"'Archivo',system-ui,sans-serif",cursor:"pointer"}}>
                  {MMM_GRADES.map(g=><option key={g.id} value={g.id} style={{background:"#1C1F25"}}>{g.label}</option>)}
                </select>
              </div>
              <div>
                ...
                <select value={mmmVoiceId} onChange={e=>setMmmVoiceId(e.target.value)}
                  style={{width:"100%",background:"#1C1F25",border:"1px solid "+GOLDDIM,color:GOLD,padding:"9px 12px",fontSize:12,fontFamily:"'Archivo',system-ui,sans-serif",cursor:"pointer"}}>
                  {VOICE_CHARACTERS.map(v=><option key={v.id} value={v.id} style={{background:"#1C1F25"}}>{v.name}</option>)}
                </select>
              </div>
            </div>

            <div style={{display:"flex",flexWrap:"wrap",gap:8,marginBottom:12}}>
              <button onClick={()=>setMmmEnhance(v=>!v)} style={{flex:"1 1 45%",padding:"10px",background:mmmEnhance?GOLD:"#1C1F25",color:mmmEnhance?"#000":GOLD,border:"1px solid "+GOLDDIM,borderRadius:5,fontWeight:600,fontSize:12,letterSpacing:0,cursor:"pointer"}}>Enhance {mmmEnhance?"ON":"OFF"}</button>
              <button onClick={()=>setMmmLipSync(v=>!v)} style={{flex:"1 1 45%",padding:"10px",background:mmmLipSync?GOLD:"#1C1F25",color:mmmLipSync?"#000":GOLD,border:"1px solid "+GOLDDIM,borderRadius:5,fontWeight:600,fontSize:12,letterSpacing:0,cursor:"pointer"}}>Lip sync {mmmLipSync?"ON":"OFF"}</button>
              <button onClick={()=>setMmmBgSound(v=>!v)} style={{flex:"1 1 45%",padding:"10px",background:mmmBgSound?GOLD:"#1C1F25",color:mmmBgSound?"#000":GOLD,border:"1px solid "+GOLDDIM,borderRadius:5,fontWeight:600,fontSize:12,letterSpacing:0,cursor:"pointer"}}>Music bed {mmmBgSound?"ON":"OFF"}</button>
            </div>
            ...
            {mmmLipSync&&(
              <div style={{marginTop:12,padding:12,border:"1px solid "+GOLDDIM+"66",borderRadius:6,background:"#1C1F25"}}>
                ...
                {mmmLsVideo&&(
                  <video src={mmmLsVideo} controls playsInline style={{width:"100%",marginTop:10,borderRadius:6,border:"1px solid "+GOLDDIM,background:"#1C1F25",aspectRatio:"16/9"}}/>
                )}
              </div>
            )}
            ...
            {mmmBusy&&(
              <div style={{marginTop:12}}>
                ...
                <div style={{background:"#1C1F25",height:10,border:"1px solid "+GOLDDIM,borderRadius:4,overflow:"hidden"}}>
            ...
            {mmmDone&&(
              <div style={{marginTop:16}}>
                ...
                <video ref={mmmVideoRef} src={mmmFilmUrl} controls autoPlay playsInline
                  style={{width:"100%",borderRadius:8,border:"1px solid "+GOLDDIM,background:"#1C1F25",aspectRatio:"16/9"}}/>
                <div style={{display:"flex",gap:10,marginTop:12}}>
                  <button onClick={mmmStitchFilm} disabled={mmmStitchBusy}
                    style={{flex:1,padding:14,background:mmmStitchBusy?"#333":GOLD,color:mmmStitchBusy?"#888":"#000",border:"none",fontWeight:600,fontSize:15,letterSpacing:0.2,borderRadius:6,cursor:mmmStitchBusy?"default":"pointer",fontFamily:"'Archivo',system-ui,sans-serif"}}>
                    {mmmStitchBusy?"STITCHING...":"DOWNLOAD FULL FILM"}
                  </button>
                  <button onClick={()=>{try{navigator.share?navigator.share({title:"My Movie",url:mmmFilmUrl}):window.open(mmmFilmUrl,"_blank");}catch(e){window.open(mmmFilmUrl,"_blank");}}}
                    style={{flex:1,padding:14,background:"#1C1F25",color:GOLD,border:"1px solid "+GOLDDIM,fontWeight:600,fontSize:15,letterSpacing:0.2,borderRadius:6,cursor:"pointer",fontFamily:"'Archivo',system-ui,sans-serif"}}>
                    EXPORT
                  </button>
                </div>
                <button onClick={mmmDownloadAll}
                  style={{width:"100%",marginTop:8,padding:10,background:"transparent",color:DIM,border:"1px solid "+GOLDDIM,fontWeight:500,fontSize:12,letterSpacing:0.2,borderRadius:6,cursor:"pointer",fontFamily:"'Archivo',system-ui,sans-serif"}}>
                  Or download scenes separately
                </button>
                {mmmStitchBusy&&(
                  <div style={{marginTop:12}}>
                    <div style={{color:GOLD,fontSize:12,letterSpacing:0,marginBottom:6}}>{mmmStitchStage}</div>
                    <div style={{background:"#1C1F25",height:10,border:"1px solid "+GOLDDIM,borderRadius:4,overflow:"hidden"}}>
                      <div style={{background:GOLD,height:"100%",width:mmmStitchPct+"%",transition:"width .3s"}}/>
                    </div>
                    <div style={{color:DIM,fontSize:10,marginTop:4}}>Stitching runs in real time - a 10-min film takes about 10 min. Keep this tab open.</div>
                  </div>
                )}
              </div>
            )}
      ...
      {/* ââââ MAKE MY MOVIE â one-shot, everything on this page ââââ */}
      <div style={{margin:"14px 20px",background:"#1C1F25",border:"2px solid "+SIGNAL,borderRadius:10,padding:16,boxShadow:"none"}}>
        <div style={{textAlign:"center",marginBottom:12}}>
          ...
          <div style={{color:DIM,fontSize:11,letterSpacing:0.2,marginTop:3}}>Drop everything in Â· set the length Â· press go â it does the rest</div>
        </div>

        <textarea value={mmmText} onChange={e=>setMmmText(e.target.value)} rows={4}
          placeholder="Paste your whole film here â script, producer instructions, prompts, notes. Or drag files onto the box below."
          style={{width:"100%",boxSizing:"border-box",background:"#1C1F25",border:"1px solid "+GOLDDIM,borderRadius:6,color:"#fff",padding:11,fontSize:13,fontFamily:"'Archivo',system-ui,sans-serif",resize:"vertical",marginBottom:8}}/>
          ...
          <div style={{display:"flex",flexWrap:"wrap",gap:5,marginBottom:10}}>
            {mmmImages.map((im,i)=>(
              <div key={i} style={{position:"relative"}}>
                <img src={im.dataUrl} style={{width:54,height:54,objectFit:"cover",border:"1px solid "+GOLDDIM,borderRadius:4}}/>
                <button onClick={()=>setMmmImages(p=>p.filter((_,x)=>x!==i))}
                  style={{position:"absolute",top:-6,right:-6,width:18,height:18,borderRadius:9,background:"#1C1F25",color:GOLD,border:"1px solid "+GOLDDIM,fontSize:11,cursor:"pointer",lineHeight:"16px",padding:0}}>Ã</button>
              </div>
            ))}
          </div>
        )}
        ...
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:5}}>
          <span style={{color:GOLD,fontSize:12,letterSpacing:0.2,fontWeight:600}}>Movie length</span>
          <span style={{color:"#fff",fontSize:12,fontWeight:500}}>{mmmTargetMin} Min Â· ~{Math.max(1,Math.round(mmmTargetMin))} Scenes</span>
        </div>
        <input type="range" min={1} max={180} value={mmmTargetMin}
          onChange={e=>setFilmDuration&&setFilmDuration(Number(e.target.value))}
          style={{width:"100%",accentColor:GOLD,marginBottom:2}}/>
        ...
        <button onClick={makeMyMovie} disabled={mmmBusy}
          ...
          {mmmBusy?"MAKING YOUR MOVIEâ¦":"Make my movie"}
        </button>
        ...
        {mmmScenes.length>0&&(
          <div style={{marginTop:10,maxHeight:120,overflowY:"auto",fontSize:11}}>
            {mmmScenes.map((s,i)=>(
              <div key={i} style={{display:"flex",gap:6,padding:"2px 0",color:s.status==="done"?GOLD:s.status==="rendering"?"#fff":s.status==="skipped"?"#ff8a8a":DIM}}>
                <span>{s.status==="done"?"â":s.status==="rendering"?"â":s.status==="skipped"?"â":"â"}</span>
                <span style={{whiteSpace:"nowrap",overflow:"hidden",textOverflow:"ellipsis"}}>Scene {i+1}: {s.text}</span>
              </div>
            ))}
          </div>
        ...
        {mmmDone&&(
          <div style={{marginTop:14}}>
            <video ref={mmmVideoRef} src={mmmFilmUrl} controls autoPlay playsInline
              style={{width:"100%",borderRadius:8,border:"1px solid "+GOLDDIM,background:"#1C1F25",aspectRatio:"16/9"}}/>
            <button onClick={mmmStitchFilm} disabled={mmmStitchBusy}
              style={{width:"100%",padding:14,marginTop:10,background:mmmStitchBusy?"#333":GOLD,color:mmmStitchBusy?"#888":"#000",border:"none",fontWeight:600,fontSize:16,letterSpacing:0.2,borderRadius:6,cursor:mmmStitchBusy?"default":"pointer",fontFamily:"'Archivo',system-ui,sans-serif"}}>
              {mmmStitchBusy?"STITCHING YOUR FILM...":"Download my movie"}
            </button>
            <button onClick={()=>{try{navigator.share?navigator.share({title:"My Movie",url:mmmFilmUrl}):window.open(mmmFilmUrl,"_blank");}catch(e){window.open(mmmFilmUrl,"_blank");}}}
              style={{width:"100%",padding:12,marginTop:8,background:"#1C1F25",color:GOLD,border:"1px solid "+GOLDDIM,fontWeight:600,fontSize:14,letterSpacing:0.2,borderRadius:6,cursor:"pointer",fontFamily:"'Archivo',system-ui,sans-serif"}}>
              EXPORT
            </button>
            <button onClick={mmmDownloadAll}
              style={{width:"100%",marginTop:8,padding:9,background:"transparent",color:DIM,border:"1px solid "+GOLDDIM,fontWeight:500,fontSize:12,letterSpacing:0.2,borderRadius:6,cursor:"pointer",fontFamily:"'Archivo',system-ui,sans-serif"}}>
              Or download scenes separately
            </button>
            {mmmStitchBusy&&(
              <div style={{marginTop:12}}>
                ...
                <div style={{color:DIM,fontSize:10,marginTop:4}}>Stitching runs in real time - keep this tab open.</div>
              </div>
            )}
      ...
      <div style={{display:"grid",gridTemplateColumns:"1fr 420px",minHeight:"calc(100vh - 120px)"}}>
        <div style={{padding:20,overflowY:"auto"}}>
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:12}}>
            <div>
              ...
              <select value={renderStyle} onChange={e=>setRenderStyle(e.target.value)}
                style={{width:"100%",background:"#1C1F25",border:"1px solid "+GOLDDIM,color:GOLD,padding:"9px 12px",fontSize:12,outline:"none",fontFamily:"'Archivo',system-ui,sans-serif",cursor:"pointer"}}>
                {RENDER_STYLES.map(s=><option key={s.id} value={s.id} style={{background:"#1C1F25"}}>{s.label}</option>)}
              </select>
            </div>
            <div>
              ...
              <select value={genre} onChange={e=>setGenre(e.target.value)}
                style={{width:"100%",background:"#1C1F25",border:"1px solid "+GOLDDIM,color:genre?GOLD:GOLDDIM,padding:"9px 12px",fontSize:12,outline:"none",fontFamily:"'Archivo',system-ui,sans-serif",cursor:"pointer"}}>
                {FILM_GENRES.map(g=><option key={g.id} value={g.id} style={{background:"#1C1F25"}}>{g.label}</option>)}
              </select>
            </div>
          </div>
          <div style={{background:"#1C1F25",border:"2px solid "+SIGNAL,padding:14,marginBottom:12,boxShadow:"none"}}>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:8}}>
              <div>
                <div style={{color:GOLD,fontSize:12,letterSpacing:0.2,fontWeight:600}}>Reality engine â upload your photos</div>
              ...
              <div style={{display:"grid",gridTemplateColumns:"repeat(6,1fr)",gap:4,marginBottom:8}}>
                {refImages.map((ri,i)=>(
                  <div key={i} style={{position:"relative"}}>
                    ...
                    <button onClick={()=>setRefImages(p=>p.filter((_,j)=>j!==i))} style={{position:"absolute",top:1,right:1,background:"#1C1F25",border:"1px solid "+GOLDDIM,color:GOLD,padding:"0 4px",cursor:"pointer",fontSize:9,fontWeight:600,lineHeight:1.2}}>â</button>
            ...
            <button onClick={()=>{if(refImages.length>=6){alert("Max 6 photos");return;}realityPhotoRef.current&&realityPhotoRef.current.click();}} style={{width:"100%",background:GOLD,border:"none",color:"#000",padding:"10px",cursor:"pointer",fontSize:11,fontWeight:600,letterSpacing:0.2,fontFamily:"'Archivo',system-ui,sans-serif"}}>
              ð· {refImages.length===0?"ADD PHOTOS / VIDEOS (UP TO 6)":"ADD MORE â "+refImages.length+"/6 LOADED"}
            </button>
            <a href="https://photos.google.com" target="_blank" rel="noopener noreferrer"
              style={{display:"block",background:"#1C1F25",border:"1px solid "+GOLDDIM,color:GOLDDIM,padding:"8px",textAlign:"center",fontSize:10,fontWeight:600,letterSpacing:0.2,textDecoration:"none",fontFamily:"'Archivo',system-ui,sans-serif",marginTop:4}}>
              CHROMEBOOK USERS OPEN GOOGLE PHOTOS download photo then Add Photos above
            </a>
            <div style={{color:GOLDDIM,fontSize:9,marginTop:5,letterSpacing:0,textAlign:"center"}}>1st photo = BACKGROUND Â· others = foreground layers Â· guarantees photorealistic output</div>
          </div>
          <div style={{background:"#1C1F25",border:"1px solid "+GOLDDIM,padding:12,marginBottom:12}}>
            ...
            {refMedia?(
              <div style={{position:"relative"}}>
                ...
                <button onClick={()=>{setRefMedia(null);setRefDataUrl(null);}} style={{position:"absolute",top:3,right:3,background:"#1C1F25",border:"1px solid "+GOLDDIM,color:GOLD,padding:"1px 6px",cursor:"pointer",fontSize:10,fontWeight:600}}>â</button>
                <div style={{color:"#22c55e",fontSize:9,fontWeight:600,letterSpacing:0.2,marginTop:3}}>Reference loaded</div>
              </div>
            ):(
              <div
                ...
                <div style={{color:GOLDDIM,fontSize:10,marginTop:3,letterSpacing:0}}>Jpg Â· png Â· MP4</div>
              </div>
            )}
          ...
          <input value={title} onChange={e=>setTitle(e.target.value)} placeholder="e.g. AI For Humanity â Chapter 1"
            style={{width:"100%",background:"#1C1F25",border:"1px solid "+GOLDDIM,padding:"10px 14px",color:WHITE,fontSize:14,outline:"none",boxSizing:"border-box",fontFamily:"'Archivo',system-ui,sans-serif",marginBottom:14}}/>
          <div style={{marginBottom:14}}>
            ...
            <textarea value={prompt} onChange={e=>setPrompt(e.target.value)}
              ...
              style={{width:"100%",background:"#1C1F25",border:"1px solid "+GOLDDIM,padding:"12px 14px",color:WHITE,fontSize:13,outline:"none",boxSizing:"border-box",fontFamily:"'Archivo',system-ui,sans-serif",lineHeight:1.9,height:140,resize:"none"}}/>
          </div>
          <div style={{marginBottom:14}}>
            <div style={{color:GOLD,fontSize:11,letterSpacing:0.2,fontWeight:600,marginBottom:8}}>Quick examples â click to try</div>
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:6}}>
              {EXAMPLES.map((ex,i)=>(
                <div key={i} onClick={()=>setPrompt(ex)}
                  style={{background:"#1C1F25",border:"1px solid "+GOLDDIM,padding:"10px 12px",cursor:"pointer",fontSize:11,color:DIM,lineHeight:1.6}}>
          ...
          <div style={{background:"#1C1F25",border:"1px solid "+GOLDDIM,padding:14,marginBottom:14}}>
            ...
            {targetMin>0&&(
              <div style={{color:GOLDDIM,fontSize:10,marginTop:6,letterSpacing:0}}>
                Made so far: {fmtMin(madeSeconds)} of {targetMin}m Â· {gapSeconds>0?("gap "+fmtMin(gapSeconds)):"target reached"}
              </div>
            )}
          </div>
          <div style={{background:"#1C1F25",border:"1px solid "+GOLDDIM,padding:14,marginBottom:14}}>
            ...
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
              ...
              <div style={{display:"flex",gap:6}}>
                <button onClick={()=>setAddMusic(true)}
                  style={{background:addMusic?GOLD:"#1C1F25",border:"1px solid "+(addMusic?"#000":GOLDDIM),color:addMusic?"#000":WHITE,padding:"5px 16px",cursor:"pointer",fontSize:11,fontWeight:600,letterSpacing:0}}>Y</button>
                <button onClick={()=>{setAddMusic(false);setMusicTrack("");}}
                  style={{background:!addMusic?GOLD:"#1C1F25",border:"1px solid "+(!addMusic?"#000":GOLDDIM),color:!addMusic?"#000":WHITE,padding:"5px 16px",cursor:"pointer",fontSize:11,fontWeight:600,letterSpacing:0}}>N</button>
              </div>
            </div>
            {addMusic&&(
              <div style={{marginTop:12}}>
                ...
                <div style={{display:"flex",gap:8,alignItems:"center"}}>
                  <select value={musicTrack} onChange={e=>setMusicTrack(e.target.value)}
                    style={{flex:1,background:"#1C1F25",border:"1px solid "+GOLDDIM,color:WHITE,padding:"9px 12px",fontSize:12,fontWeight:600,fontFamily:"'Archivo',system-ui,sans-serif",outline:"none",cursor:"pointer"}}>
                    <option value="">â Select background music â</option>
                    {MUSIC_LIBRARY.map(m=>(<option key={m.id} value={m.id} style={{background:"#1C1F25",color:WHITE}}>{m.label}</option>))}
                  </select>
                  <button onClick={()=>{const m=MUSIC_LIBRARY.find(x=>x.id===musicTrack);if(!m)return;try{const a=new Audio(m.url);a.volume=0.5;a.play().catch(()=>{});setTimeout(()=>{try{a.pause();}catch(e){}},6000);}catch(e){}}}
                    ...
                    style={{background:musicTrack?"none":"#1C1F25",border:"1px solid "+GOLDDIM,color:musicTrack?GOLD:"#555",padding:"9px 14px",cursor:musicTrack?"pointer":"not-allowed",fontSize:12,fontWeight:600}}>Preview</button>
                </div>
          ...
          <div onClick={()=>setGenStereo(s=>!s)} style={{display:"flex",alignItems:"center",gap:10,marginBottom:14,padding:"12px 14px",background:"#1C1F25",border:"1px solid "+(genStereo?GOLD:GOLDDIM),cursor:"pointer"}}>
            <div style={{width:20,height:20,borderRadius:4,border:"2px solid "+(genStereo?GOLD:GOLDDIM),background:genStereo?GOLD:"transparent",color:"#000",fontWeight:600,fontSize:14,display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0}}>{genStereo?"â":""}</div>
          ...
          {hasBrief&&(
            <div onClick={()=>setUseBrief(b=>!b)} style={{display:"flex",alignItems:"center",gap:10,marginBottom:14,padding:"12px 14px",background:"#1C1F25",border:"1px solid "+(useBrief?GOLD:GOLDDIM),cursor:"pointer"}}>
              <div style={{width:20,height:20,borderRadius:4,border:"2px solid "+(useBrief?GOLD:GOLDDIM),background:useBrief?GOLD:"transparent",color:"#000",fontWeight:600,fontSize:14,display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0}}>{useBrief?"â":""}</div>
          ...
          <button onClick={generateVideo} disabled={generating||!prompt.trim()}
            style={{background:"#1C1F25",border:"none",color:"#000",width:"100%",padding:"20px",fontSize:15,letterSpacing:0.2,cursor:generating||!prompt.trim()?"not-allowed":"pointer",fontWeight:600,fontFamily:"'Archivo',system-ui,sans-serif",opacity:generating||!prompt.trim()?0.5:1}}>
            {generating?"â³ MANDASTRONG ENGINE RENDERING... "+progress+"%":"Generate scene"}
          </button>
        </div>
        <div style={{borderLeft:"1px solid "+GOLDDIM+"",display:"flex",flexDirection:"column"}}>
          <div style={{background:"#1C1F25",aspectRatio:"16/9",display:"flex",alignItems:"center",justifyContent:"center",borderBottom:"1px solid "+GOLDDIM+"",overflow:"hidden"}}>
            {videoUrl?(
          ...
          {generating&&(
            <div style={{padding:"10px 14px",borderBottom:"1px solid "+GOLDDIM+""}}>
              <div style={{height:5,background:"#1C1F25",marginBottom:4}}>
                <div style={{width:progress+"%",height:"100%",background:"#1C1F25",transition:"width .4s"}}/>
              </div>
            ...
            <div style={{padding:"10px 14px",borderBottom:"1px solid "+GOLDDIM+"",display:"flex",flexDirection:"column",gap:6}}>
              <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:6}}>
                ...
                <button onClick={saveToLibrary}
                  style={{background:saved?"#1C1F25":"transparent",border:"1px solid "+GOLDDIM,color:saved?"#000":GOLD,padding:"8px",fontSize:10,cursor:"pointer",fontWeight:600,letterSpacing:0,fontFamily:"'Archivo',system-ui,sans-serif"}}>
                  {saved?"Saved":"Library"}
                </button>
              </div>
              <button onClick={()=>{setVideoUrl("");setLog([]);setSaved(false);setTitle("");setPrompt("");}}
                style={{background:"#1C1F25",border:"none",color:"#000",padding:"8px",fontSize:11,width:"100%",letterSpacing:0.2,cursor:"pointer",fontWeight:600,fontFamily:"'Archivo',system-ui,sans-serif"}}>
                Next scene
              </button>
            </div>
          )}
...
// photograph loads instead â never a broken image, never a cartoon.
// ââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââ
...
function Frame({ seed, local, vid, localVid, label, sub, dur, h=160, onClick }) {
  const chain = [local?("frames/"+local):null, STOCK(seed,900,506), STOCK(seed+"2",900,506)].filter(Boolean);
  const [i,setI] = useState(0);
  const src = chain[i];
  const vchain = [localVid?("frames/"+localVid):null, vid?CLIP(vid):null].filter(Boolean);
  const [vi,setVi] = useState(0);
  const [vdead,setVdead] = useState(false);
  const vsrc = vdead ? null : vchain[vi];
  return (
    <div onClick={onClick} style={{position:"relative",height:h,borderRadius:10,overflow:"hidden",background:PANEL,border:"1px solid "+GOLDDIM,cursor:onClick?"pointer":"default"}}>
      <style>{"@keyframes msfpan{0%{transform:scale(1.05)}100%{transform:scale(1.15) translate3d(-2%,-2%,0)}}"}</style>
      {src&&<img src={src} alt={label||"Example frame"} loading="lazy" onError={()=>setI(v=>v+1)}
        style={{width:"100%",height:"100%",objectFit:"cover",display:"block",animation:"msfpan 20s ease-in-out infinite alternate"}}/>}
      {vsrc&&<video key={vsrc} src={vsrc} autoPlay muted loop playsInline preload="metadata"
      ...
      {onClick&&<div style={{position:"absolute",left:"50%",top:"46%",transform:"translate(-50%,-50%)",width:38,height:38,borderRadius:"50%",background:"rgba(12,13,16,.6)",border:"1px solid "+SIGNAL,display:"flex",alignItems:"center",justifyContent:"center",color:SIGNAL,fontSize:13,pointerEvents:"none"}}>â¶</div>}
    </div>
  );
}

const REEL = [
  {seed:"msf-doc",vid:"TearsOfSteel.mp4",localVid:"doc.mp4",local:"doc.jpg",label:"Documentary",sub:"Narration Â· archive grade",dur:"60 min"},
  {seed:"msf-feature",vid:"ForBiggerEscapes.mp4",localVid:"feature.mp4",local:"feature.jpg",label:"Feature drama",sub:"Multi-scene Â· 4K",dur:"90 min"},
  {seed:"msf-music",vid:"ForBiggerBlazes.mp4",localVid:"music.mp4",local:"music.jpg",label:"Music video",sub:"Beat-synced cuts",dur:"3 min"},
  {seed:"msf-short",vid:"ForBiggerJoyrides.mp4",localVid:"short.mp4",local:"short.jpg",label:"Short film",sub:"Single narrative arc",dur:"10 min"},
  {seed:"msf-family",vid:"ForBiggerFun.mp4",localVid:"family.mp4",local:"family.jpg",label:"Family film",sub:"Warm grade Â· gentle pacing",dur:"30 min"},
  {seed:"msf-trailer",vid:"ForBiggerMeltdowns.mp4",localVid:"trailer.mp4",local:"trailer.jpg",label:"Trailer",sub:"Fast cut Â· title cards",dur:"90 sec"},
];
  ...
  return (
    <div style={{padding:"26px 24px 30px",borderTop:"1px solid "+GOLDDIM}}>
      ...
      <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(210px,1fr))",gap:12}}>
        {REEL.map(f=>(
          <Frame key={f.seed} seed={f.seed} local={f.local} label={f.label} sub={f.sub} dur={f.dur} h={132} onClick={go?()=>go(5):null}/>
        ))}
      </div>
...
function P1({ go }) {
  return (
    <div style={{...Sp}}>
      <div style={{background:"#1C1F25",padding:"56px 40px 36px",textAlign:"center",position:"relative",overflow:"hidden"}}>
        ...
        <div style={{position:"relative",zIndex:1}}>
          <div style={{fontSize:11,color:DIM,letterSpacing:0.4,marginBottom:12}}>Cinema intelligence platform â est. 2025</div>
          ...
          <div style={{color:WHITE,fontSize:12,letterSpacing:0.4,marginBottom:28,fontWeight:600}}>600+ AI tools Â· 8K export Â· up to 3-hour films</div>
      ...
      <div style={{textAlign:"center",paddingBottom:24,paddingTop:16}}>
        <div style={{display:"flex",flexDirection:"column",alignItems:"center",gap:8}}>
          <button onClick={async()=>{
            ...
            try{
              const APP_URL="https://mandsstrongmovie.bolt.host";
            ...
            if(window.deferredInstallPrompt){
              ...
              alert("â MandaStrong Studio.html downloaded to your device.\n\nTo also add it to your home screen on iPhone/iPad:\n1. Tap the Share button â¬ in Safari\n2. Scroll down and tap 'Add to Home Screen'\n3. Tap 'Add'");
            } else if(isAndroid){
              alert("â MandaStrong Studio downloaded.\n\nTo also install it as an app:\n1. Tap the menu â® in Chrome\n2. Tap 'Add to Home screen' or 'Install app'");
            } else {
              alert("â MandaStrong Studio.html downloaded to your computer.\n\nDouble-click the file any time to open the studio full-screen.\n\nTo also install it: look for the install icon â in your browser's address bar.");
            }
...
function P2({ go }) {
  const pipeline=[{n:"01",ic:"â",t:"Write",d:"Script, logline, scenes",p:5},{n:"02",ic:"",t:"Voice",d:"54 AI voice characters",p:6},{n:"03",ic:"",t:"Image",d:"AI-generated stills",p:7},{n:"04",ic:"",t:"Video",d:"Cinema scene engine",p:8},{n:"05",ic:"â±",t:"Timeline",d:"Multi-track editor",p:13},{n:"06",ic:"",t:"MIX",d:"4-channel audio mixer",p:15},{n:"07",ic:"",t:"Render",d:"Up to 4K export",p:16}];
  const templates=[{seed:"msf-feature",vid:"ForBiggerEscapes.mp4",localVid:"feature.mp4",local:"feature.jpg",dur:"90 min",t:"Feature film",d:"90-minute drama.",pages:[5,6,8,13,15,16],bg:"#22262D"},{seed:"msf-doc",vid:"TearsOfSteel.mp4",localVid:"doc.mp4",local:"doc.jpg",dur:"60 min",t:"Documentary",d:"60-minute documentary.",pages:[5,6,8,13,15,16],bg:"#1C1F25"},{seed:"msf-music",vid:"ForBiggerBlazes.mp4",localVid:"music.mp4",local:"music.jpg",dur:"3 min",t:"Music video",d:"Beat-synced cinematic video.",pages:[6,8,13,16],bg:"#1C1F25"},{seed:"msf-short",vid:"ForBiggerJoyrides.mp4",localVid:"short.mp4",local:"short.jpg",dur:"10 min",t:"Short film",d:"10-minute narrative.",pages:[5,6,8,13,16],bg:"#1C1F25"},{seed:"msf-family",vid:"ForBiggerFun.mp4",localVid:"family.mp4",local:"family.jpg",dur:"30 min",t:"Family movie",d:"30-minute family film.",pages:[5,6,8,13,16],bg:"#22262D"},{seed:"msf-audio",vid:"VolkswagenGTIReview.mp4",localVid:"audio.mp4",local:"audio.jpg",dur:"Audio",t:"Audiobook",d:"Narrated audiobook.",pages:[5,6,15,16],bg:"#1C1F25"}];
  return(
    <div style={{...Sp,padding:"0 0 40px"}}>
      <div style={{padding:"20px 24px 14px",display:"flex",alignItems:"center",justifyContent:"space-between",flexWrap:"wrap",gap:10}}>
        <div><div style={{fontSize:10,color:GOLD,letterSpacing:0.4,fontWeight:500,marginBottom:4}}>Mandastrong studio Â· cinema intelligence platform</div><h1 style={{fontFamily:"'Archivo',system-ui,sans-serif",color:GOLD,fontSize:"clamp(22px,4vw,40px)",fontWeight:600,letterSpacing:0.4,margin:0}}>Studio dashboard</h1></div>
        ...
        <div style={{display:"flex",gap:0,overflowX:"auto"}}>
          {pipeline.map((step,i)=>(
            <div key={step.n} style={{display:"flex",alignItems:"center",flexShrink:0}}>
              <div onClick={()=>go(step.p)} style={{background:"#1C1F25",border:"1px solid "+GOLDDIM,padding:"14px 16px",cursor:"pointer",textAlign:"center",minWidth:120}}
...
function P3() {
  ...
  const inp={width:"100%",background:"#1C1F25",border:"1px solid "+GOLDDIM,padding:"8px 10px",color:WHITE,fontSize:12,outline:"none",fontFamily:"'Archivo',system-ui,sans-serif",boxSizing:"border-box"};
  ...
  return (
    <div style={{...Sp,padding:40}}>
      <div style={{maxWidth:1100,margin:"0 auto"}}>
        ...
        <div style={{color:"#22c55e",fontSize:11,marginBottom:24,letterSpacing:0.2,fontWeight:600}}>Saved permanently â your films stay until you replace or remove them</div>
        ...
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:20}}>
          {[0,1,2].map(i=>(
            <div key={i} style={{...Card(),padding:16}}>
              ...
              <div style={{background:"#1C1F25",aspectRatio:"16/9",marginBottom:10,border:"1px solid "+(uploads[i]?GOLD:GOLDDIM),overflow:"hidden",position:"relative",display:"flex",alignItems:"center",justifyContent:"center",cursor:"pointer"}}
              ...
              {!uploads[i]?(
                ...
                <div>
                  <div style={{color:"#22c55e",fontSize:9,fontWeight:600,letterSpacing:0.2,marginBottom:6}}>â {uploads[i].name.slice(0,28)} Â· {uploads[i].size}MB</div>
                  <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:6}}>
...
function P4({ go, setUser }) {
  ...
  const inp={width:"100%",background:"#1C1F25",border:"1px solid "+GOLDDIM,padding:"10px 12px",color:WHITE,fontSize:14,marginBottom:10,outline:"none",boxSizing:"border-box",fontFamily:"'Archivo',system-ui,sans-serif"};
  const login=()=>{
  ...
  return (
    <div style={{...Sp,padding:40}}>
      <div style={{maxWidth:1000,margin:"0 auto"}}>
        ...
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:18,marginBottom:36}}>
          ...
          <div style={{...Card(),border:"2px solid #22c55e",position:"relative"}}>
            ...
            <button onClick={()=>{setUser({name:name||"Creator",plan:"Studio Trial",isAdmin:false});window.open(STRIPE.studio,"_blank");go(5);}}
              style={{width:"100%",padding:"12px",background:"#22c55e",border:"none",color:"#000",fontWeight:600,fontSize:13,cursor:"pointer",letterSpacing:0.2}}>Start free trial â $0</button>
          </div>
        ...
        <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:14}}>
          {[
            ...
            <div key={plan.t} style={{...Card(),border:plan.pop?"2px solid "+SIGNAL:"1px solid "+GOLDDIM,position:"relative"}}>
              ...
              <div style={{margin:"12px 0"}}>{plan.f.map(f=><div key={f} style={{color:WHITE,fontSize:12,padding:"3px 0",borderBottom:"1px solid #0a0a0a"}}>â {f}</div>)}</div>
        ...
        <div style={{marginTop:40,padding:"28px 24px",background:"#14161A",border:"2px solid "+SIGNAL,textAlign:"center",boxShadow:"none"}}>
...
function MergeVideos({ onSave }) {
  ...
  const mergeAll = async () => {
    ...
    try {
      ...
      setMergedUrl(url);
      setProgress(100);
      log("Merge complete â"+(blob.size/1024/1024).toFixed(1)+"MB Â· "+clips.length+" clips combined");
      ...
      try {
        ...
        await safeSaveClipToDB(clipId, blob, fn, "video/webm");
        if(onSave) onSave({id:clipId, name:fn, type:"video/webm", url:URL.createObjectURL(blob), file:new File([blob],fn,{type:"video/webm"}), dbId:clipId});
        log("Saved to media library â ready for timeline");
      } catch(e) {}
  ...
  return (
    <div>
      ...
      <button onClick={()=>fileRef.current&&fileRef.current.click()}
        style={{width:"100%",background:"#1C1F25",border:"1px solid "+GOLDDIM,color:WHITE,padding:"10px",cursor:"pointer",fontSize:11,fontWeight:600,letterSpacing:0.2,fontFamily:"'Archivo',system-ui,sans-serif",marginBottom:10}}>
        â¬ ADD VIDEOS TO MERGE ({clips.length} loaded)
      </button>
      {clips.length>0&&(
        <div style={{marginBottom:10}}>
          <div style={{color:GOLD,fontSize:10,letterSpacing:0.2,fontWeight:600,marginBottom:6}}>Drag to reorder â top = first in film</div>
          {clips.map((c,i)=>(
            <div key={c.id}
              draggable
              ...
              style={{background:"#1C1F25",border:"1px solid "+GOLDDIM,padding:"8px 12px",marginBottom:4,display:"flex",alignItems:"center",gap:10,cursor:"grab"}}>
              <span style={{color:GOLD,fontWeight:600,fontSize:13}}>â£¿</span>
              ...
              <div style={{display:"flex",gap:4}}>
                {i>0&&<button onClick={()=>move(i,i-1)} style={{background:"none",border:"1px solid "+GOLDDIM,color:GOLD,padding:"2px 7px",cursor:"pointer",fontSize:10,fontWeight:600}}>â²</button>}
                {i<clips.length-1&&<button onClick={()=>move(i,i+1)} style={{background:"none",border:"1px solid "+GOLDDIM,color:GOLD,padding:"2px 7px",cursor:"pointer",fontSize:10,fontWeight:600}}>â¼</button>}
                <button onClick={()=>setClips(p=>p.filter((_,j)=>j!==i))} style={{background:"none",border:"1px solid #ef4444",color:"#ef4444",padding:"2px 7px",cursor:"pointer",fontSize:10,fontWeight:600}}>â</button>
              </div>
            </div>
          ))}
        </div>
      ...
      {clips.length>=2&&(
        <button onClick={mergeAll} disabled={merging}
          ...
          {merging?"â³ MERGING... "+progress+"%":"Merge in sequence save to timeline"}
        </button>
      )}
      {merging&&(
        <div style={{height:4,background:"#1C1F25",marginBottom:8}}>
          <div style={{width:progress+"%",height:"100%",background:SIGNAL,transition:"width .3s"}}/>
        </div>
      )}
      {mergeLog.length>0&&(
        <div style={{background:"#1C1F25",border:"1px solid "+GOLDDIM,padding:10,maxHeight:100,overflowY:"auto",marginBottom:8}}>
          {mergeLog.map((l,i)=>(
      ...
      {mergedUrl&&(
        <div style={{background:"#061406",border:"1px solid #22c55e",padding:"10px 14px"}}>
          <div style={{color:"#22c55e",fontWeight:600,fontSize:11,letterSpacing:0.2,marginBottom:6}}>Merged film saved to media library â ready for timeline</div>
  ...
  return (
    <div style={{...Sp,padding:40}}>
      <div style={{maxWidth:800,margin:"0 auto"}}>
        ...
        <div onDragOver={e=>{e.preventDefault();e.currentTarget.style.borderColor=GOLD;}}
          ...
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,maxWidth:360,margin:"0 auto"}}>
            <button onClick={()=>fileRef.current&&fileRef.current.click()}
              style={{background:"#1C1F25",border:"2px solid "+SIGNAL,color:GOLD,padding:"14px",cursor:"pointer",fontSize:13,fontWeight:600,letterSpacing:0.2,fontFamily:"'Archivo',system-ui,sans-serif"}}>
              Upload photos
            </button>
            <button onClick={()=>fileRef.current&&fileRef.current.click()}
              style={{background:"#1C1F25",border:"2px solid "+SIGNAL,color:WHITE,padding:"14px",cursor:"pointer",fontSize:13,fontWeight:600,letterSpacing:0.2,fontFamily:"'Archivo',system-ui,sans-serif"}}>
              Upload files
            </button>
          </div>
        </div>
              ...
              {mediaLib.map(a=>(
                <div key={a.id} style={{...Card(),padding:8,position:"relative"}}>
                  {a.type.startsWith("video")&&a.url?<video src={a.url} style={{width:"100%",marginBottom:5}}/>:
                   ...
                   <div style={{height:60,background:"#1C1F25",marginBottom:5,display:"flex",alignItems:"center",justifyContent:"center",fontSize:22}}></div>}
                  ...
                  <button onClick={()=>setMediaLib(p=>p.filter(x=>x.id!==a.id))}
                    style={{position:"absolute",top:5,right:5,background:"#7f1d1d",border:"none",color:"#ef4444",width:16,height:16,cursor:"pointer",fontSize:9,padding:0}}>â</button>
                </div>
              ))}
            </div>
          </div>
      ...
      <div style={{marginTop:20}}>
        <div style={{background:"#1C1F25",border:"2px solid "+SIGNAL,padding:16,marginBottom:12}}>
          <div style={{color:GOLD,fontSize:12,letterSpacing:0.2,fontWeight:600,marginBottom:6}}>Merge videos â combine & order before timeline</div>
        ...
        <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:20}}>
          {boxes.map(b=>(
            <button key={b.t} onClick={()=>go(b.p)}
              style={{background:"#1C1F25",border:"1px solid "+GOLDDIM,padding:"28px 24px",cursor:"pointer",textAlign:"left",fontFamily:"'Archivo',system-ui,sans-serif",minHeight:150,display:"flex",flexDirection:"column"}}
...
function P13({ go, mediaLib, timeline, setTimeline, user, filmDuration, setFilmDuration }) {
  ...
  return (
    <div style={{...Sp,padding:20}}>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:12,flexWrap:"wrap",gap:10}}>
        <div>
          ...
          <div style={{display:"flex",alignItems:"center",gap:8,marginTop:4}}>
            ...
            <div style={{display:"flex",gap:4}}>
              {[60,90,180].map(m=><button key={m} onClick={()=>setFilmDuration(m)} style={{background:filmDuration===m?GOLD:"#1C1F25",border:"1px solid "+(filmDuration===m?"#000":GOLDDIM),color:filmDuration===m?"#000":WHITE,padding:"2px 8px",cursor:"pointer",fontSize:10,fontWeight:600,fontFamily:"'Archivo',system-ui,sans-serif"}}>{m}m</button>)}
            </div>
          </div>
        </div>
          ...
          <button onClick={()=>{
            ...
            if(gapSec>30){
              ...
              if(wantFill){
                ...
                alert("â Fill-scene prompt copied.\n\nGo to Page 8 (Video Tools), paste it, and generate "+n+" more clip"+(n>1?"s":"")+" â then SYNC again.");
                go(8);
                return;
              }
            }
            alert("All tracks synced in order â"+videoAssets.length+" video clips Â· "+audioAssets.length+" audio tracks");
      ...
      <div style={{background:"#1C1F25",height:100,display:"flex",alignItems:"center",justifyContent:"center",marginBottom:12,border:"1px solid "+GOLDDIM}}>
      ...
      {tracks.map((tr,idx)=>(
        <div key={idx} style={{marginBottom:8}}>
          ...
          <div onDragOver={e=>{e.preventDefault();e.currentTarget.style.border="1px dashed "+GOLD;}}
            ...
            style={{background:"#1C1F25",border:"1px dashed "+GOLDDIM,minHeight:42,padding:6,display:"flex",gap:6,alignItems:"center",flexWrap:"wrap"}}>
            {(timeline[idx]||[]).map((a,i)=>(
              <div key={i} draggable
                ...
                <button onClick={()=>setTimeline(p=>({...p,[idx]:p[idx].filter((_,j)=>j!==i)}))}
                  style={{background:"none",border:"none",color:"#000",cursor:"pointer",fontSize:11,padding:0}}>â</button>
              </div>
            ))}
          ...
          <div style={{display:"flex",gap:6,flexWrap:"wrap"}}>
            {mediaLib.map(a=>(
              <div key={a.id} draggable onDragStart={e=>e.dataTransfer.setData("assetId",String(a.id))}
                style={{background:"#1C1F25",border:"1px solid "+GOLDDIM,padding:"4px 10px",cursor:"grab",color:GOLD,fontSize:12,fontWeight:500}}>
                ð {a.name.slice(0,14)}
              </div>
            ))}
          </div>
      ...
      <div style={{...Card(),marginTop:12,display:"flex",alignItems:"center",gap:8}}>
        ...
        <div style={{flex:1,height:3,background:"#1C1F25"}}/>
...
function P14() {
  ...
  return (
    <div style={{...Sp,display:"flex"}}>
      <div style={{width:176,background:"#14161A",borderRight:"1px solid "+GOLDDIM+"",overflowY:"auto",padding:8}}>
        {tools14.map(t=>(
        ...
        <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:12,marginBottom:24}}>
          {Object.entries(lvl).map(([ch,val])=>(
            <div key={ch} style={{...Card(),textAlign:"center",padding:18}}>
              ...
              <div style={{height:3,background:"#1C1F25",marginTop:10}}>
...
function P16({ go, timeline, setRendered, mediaLib, setMediaLib, user, filmDuration, setFilmDuration }) {
  ...
  const startRender=async()=>{
    ...
    try{
      ...
      // stays on top (locked mix VOICE 85 / MUSIC 40 â 0.25 gain under voice).
        ...
        try{
          ...
          await new Promise(resolve=>{
            ...
            const tick=()=>{
              if(frame>=totalFrames){resolve(null);return;}
              const t=frame/totalFrames,sec=frame/fps;
              try{ctx.clearRect(0,0,W,H);drawFn(ctx,W,H,t,sec);}catch(e){ctx.fillStyle="#14161A";ctx.fillRect(0,0,W,H);}
              const vig=ctx.createRadialGradient(W/2,H/2,W*0.1,W/2,H/2,W*0.8);
              vig.addColorStop(0,"rgba(0,0,0,0)");vig.addColorStop(1,"rgba(0,0,0,0.85)");
              ctx.fillStyle=vig;ctx.fillRect(0,0,W,H);
              ctx.fillStyle="#000";ctx.fillRect(0,0,W,H*0.06);ctx.fillRect(0,H*0.94,W,H*0.06);
      ...
      if(targetTotal>0 && clips.length>0){
        if(gapFill){
          ...
          log("Stretch mode: film "+(targetTotal/60).toFixed(1)+" min Ã· "+clips.length+" clips â "+perClipTarget.toFixed(1)+"s each");
        }
      }
  ...
  return (
    <div style={{...Sp,padding:0}}>
      ...
      <div style={{padding:"12px 24px",borderBottom:"1px solid "+GOLDDIM+"",background:"#020200",display:"flex",alignItems:"center",justifyContent:"space-between",flexWrap:"wrap",gap:10}}>
        <div>
          <div style={{fontSize:10,color:GOLD,letterSpacing:0.4,fontWeight:500}}>Production engine â stage 6</div>
      ...
      <div style={{display:"grid",gridTemplateColumns:"1fr 320px",minHeight:"calc(100vh - 120px)"}}>
        <div style={{padding:20,overflowY:"auto"}}>
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:16}}>
            <div style={{background:clips.length>0?"#061406":"#1C1F25",border:"1px solid "+(clips.length>0?"#22c55e":GOLDDIM),padding:"14px 16px"}}>
              <div style={{color:GOLD,fontSize:10,fontWeight:600,letterSpacing:0.2,marginBottom:6}}>Video clips</div>
              <div style={{color:clips.length>0?"#22c55e":WHITE,fontSize:14,fontWeight:600}}>{clips.length>0?"â "+clips.length+" clip"+(clips.length>1?"s":"")+" ready":"No clips â generate on page 8"}</div>
            </div>
            <div style={{background:audio?"#061406":"#1C1F25",border:"1px solid "+(audio?"#22c55e":GOLDDIM),padding:"14px 16px"}}>
              <div style={{color:GOLD,fontSize:10,fontWeight:600,letterSpacing:0.2,marginBottom:6}}>Audio track</div>
              <div style={{color:audio?"#22c55e":"#f59e0b",fontSize:14,fontWeight:600}}>{audio?"Audio ready":"No audio â record on page 6"}</div>
            </div>
          </div>
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:16}}>
            <div style={{background:"#1C1F25",border:"1px solid "+GOLDDIM,padding:"14px 16px"}}>
              <div style={{color:GOLD,fontSize:10,fontWeight:600,letterSpacing:0.2,marginBottom:10}}>Output quality</div>
              <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:5}}>
                {QUALITIES.map(q=>(
                  <button key={q.id} onClick={()=>setQuality(q.id)} style={{background:quality===q.id?"#1C1F25":"#000",border:"1px solid "+(quality===q.id?GOLD:GOLDDIM),padding:"8px 6px",cursor:"pointer",textAlign:"center"}}>
          ...
          {rendering&&(
            <div style={{background:"#1C1F25",border:"1px solid "+GOLDDIM,padding:"14px 16px",marginBottom:16}}>
              ...
              <div style={{height:8,background:"#1C1F25",overflow:"hidden"}}>
                <div style={{width:progress+"%",height:"100%",background:SIGNAL,transition:"width .3s"}}/>
              </div>
            </div>
          )}
          {renderLog.length>0&&(
            <div style={{background:"#1C1F25",border:"1px solid "+GOLDDIM,padding:"14px 16px",marginBottom:16,maxHeight:180,overflowY:"auto"}}>
              ...
              <div style={{display:"flex",gap:10,flexWrap:"wrap"}}>
                <button onClick={()=>{
                  try{
                    ...
                    log("Download started â check your device's Downloads");
            ...
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
              ...
              <div style={{display:"flex",gap:6,flexShrink:0,marginLeft:12}}>
                <button onClick={()=>setGapFill(true)}
                  style={{background:gapFill?GOLD:"#1C1F25",border:"1px solid "+(gapFill?"#000":GOLDDIM),color:gapFill?"#000":WHITE,padding:"6px 18px",cursor:"pointer",fontSize:12,fontWeight:600,letterSpacing:0}}>Y</button>
                <button onClick={()=>setGapFill(false)}
                  style={{background:!gapFill?GOLD:"#1C1F25",border:"1px solid "+(!gapFill?"#000":GOLDDIM),color:!gapFill?"#000":WHITE,padding:"6px 18px",cursor:"pointer",fontSize:12,fontWeight:600,letterSpacing:0}}>N</button>
              </div>
            </div>
          </div>
          <div style={{background:"#14161A",border:"2px solid "+SIGNAL,padding:"18px 20px",marginBottom:16}}>
        ...
        <div style={{borderLeft:"1px solid "+GOLDDIM+"",display:"flex",flexDirection:"column",background:"#020200"}}>
          <div style={{background:"#1C1F25",aspectRatio:"16/9",display:"flex",alignItems:"center",justifyContent:"center",overflow:"hidden"}}>
            {renderUrl?(
          ...
          <div style={{flex:1,overflowY:"auto",padding:14}}>
            ...
            {clips.length===0?(
              ...
              <div key={clip.id||i} style={{display:"flex",alignItems:"center",gap:8,padding:"8px 10px",marginBottom:4,background:currentClipIdx===i?"#1C1F25":"#1C1F25",border:"1px solid "+(currentClipIdx===i?GOLD:GOLDDIM)}}>
  ...
  return (
    <div style={{...Sp,padding:40}}>
      <div style={{maxWidth:880,margin:"0 auto"}}>
        ...
        <div style={{background:"#1C1F25",overflow:"hidden",marginBottom:14,aspectRatio:"16/9",display:"flex",alignItems:"center",justifyContent:"center",border:"1px solid "+GOLDDIM}}>
        ...
        <div style={{...Card(),display:"flex",alignItems:"center",gap:8}}>
          ...
          <div style={{flex:1,height:4,background:"#1C1F25",cursor:"pointer"}}
        ...
        <div style={{display:"flex",gap:8,flexWrap:"wrap"}}>
          {[["YouTube","#FF0000","https://www.youtube.com/upload"],["Instagram","#E1306C","https://www.instagram.com"],["TikTok","#69C9D0","https://www.tiktok.com/upload"],["X / Twitter","#1DA1F2","https://twitter.com/intent/tweet?text=Check+out+my+film+made+with+MandaStrong+Studio"],["Facebook","#1877F2","https://www.facebook.com/sharer/sharer.php?u=https://mandastrong1.etsy.com"],["LinkedIn","#0A66C2","https://www.linkedin.com/sharing/share-offsite/?url=https://mandastrong1.etsy.com"],["Vimeo","#1AB7EA","https://vimeo.com/upload"],["WhatsApp","#25D366","https://api.whatsapp.com/send?text=Check+out+my+film+from+MandaStrong+Studio"]].map(([s,c,link])=>(
            <button key={s} onClick={()=>window.open(link,"_blank")}
              style={{background:"#1C1F25",border:"1px solid "+GOLDDIM,padding:"10px 16px",cursor:"pointer"}}
...
function TutCanvas({drawFn}){
  ...
  return <canvas ref={cvRef} style={{width:"100%",display:"block",background:"#1C1F25",boxShadow:"0 0 60px rgba(232,201,109,0.25), inset 0 0 40px rgba(0,0,0,0.5)"}}/>;
}
...
function P19({ go }) {
  ...
  const tuts=[
    {n:"01",t:"Getting Started â Platform Overview",d:"Complete walkthrough of all 24 pages, Quick Access menu, footer controls, auto-save, and navigation.",dur:"3:00",l:"Beginner",page:1,tips:["Use â° top left to jump to any page","AUTOSAVE ON is real â state saves automatically as you work","ð¾ SAVE PROJECT saves a named session to MY PROJECTS for full restore"]},
    ...
    {n:"06",t:"Timeline Editor â Building Your Film",d:"Clips auto-populate from Media Library. Drag to reorder. Upload Media button always visible. SYNC ALL TRACKS for instant assembly.",dur:"4:00",l:"Intermediate",page:13,tips:["Hit â¡ SYNC ALL TRACKS to auto-populate all clips in order","Use â¬ UPLOAD MEDIA (next to CLEAR ALL) to add more clips at any time","Narration saves auto-populate the audio track â no dragging needed"]},
  ...
  return(
    <div style={{...Sp,padding:0,background:"#1C1F25"}}>
      <div style={{background:"#1C1F25",borderBottom:"1px solid "+GOLDDIM+"44",padding:"24px 32px 18px"}}>
        ...
        {tuts.map((t,idx)=>{
          ...
          return(
            <div key={t.n} style={{marginBottom:10}}>
              <div style={{background:isActive?"#14161A":"#14161A",border:"1px solid "+(isActive?GOLD:GOLDDIM+"66"),borderBottom:isActive?"none":undefined,display:"flex",alignItems:"stretch",cursor:"pointer"}}
                onClick={()=>setActive(isActive?null:idx)}>
                <div style={{width:56,flexShrink:0,display:"flex",alignItems:"center",justifyContent:"center",background:isActive?PANEL2:"#1C1F25",borderRight:"1px solid "+isActive?GOLD+"66":GOLDDIM+"33"+""}}>
                ...
                <div style={{flex:1,padding:"13px 16px",minWidth:0}}>
                  ...
                  <div style={{color:GOLDDIM,fontSize:10,letterSpacing:0}}>{t.dur} Â· {t.tips.length} Pro tips</div>
                </div>
                <div style={{display:"flex",alignItems:"center",gap:10,padding:"0 16px",flexShrink:0}}>
                  ...
                  <span style={{color:isActive?GOLD:GOLDDIM,fontSize:14,fontWeight:600}}>{isActive?"â²":"â¼"}</span>
                </div>
              </div>
              {isActive&&(
                <div style={{background:"#040300",border:"1px solid "+GOLDDIM,borderTop:"none"}}>
                  {isGen?(
                    <div style={{aspectRatio:"16/9",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",gap:16,background:"#1C1F25"}}>
                    ...
                    <div style={{aspectRatio:"16/9",background:"#1C1F25",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",gap:18,cursor:"pointer"}}
                      ...
                      <div style={{width:80,height:80,background:GOLD,display:"flex",alignItems:"center",justifyContent:"center",boxShadow:"none",cursor:"pointer"}}>
                        <span style={{color:"#000",fontSize:30,fontWeight:600,marginLeft:4}}>â¶</span>
                      </div>
                      <div style={{textAlign:"center"}}>
                        <div style={{color:GOLD,fontWeight:600,fontSize:15,letterSpacing:0.4,marginBottom:6}}>Generate to watch</div>
                        <div style={{color:GOLDDIM,fontSize:10,letterSpacing:0.2}}>Lesson {t.n} Â· {t.dur} Â· {t.l.toUpperCase()}</div>
                      </div>
                    </div>
                  )}
                  <div style={{padding:"20px 26px"}}>
                    ...
                    <div style={{marginTop:18,display:"flex",gap:10,flexWrap:"wrap"}}>
                      ...
                      <button onClick={()=>go(t.page)} style={{background:"transparent",border:"1px solid "+GOLDDIM,color:GOLD,padding:"11px 20px",cursor:"pointer",fontSize:11,fontWeight:600,letterSpacing:0.2,fontFamily:"'Archivo',system-ui,sans-serif"}}>Open page {t.page} â¶</button>
                    </div>
                  </div>
                </div>
              )}
  ...
  return (
    <div style={{...Sp,padding:40}}>
      <div style={{maxWidth:860,margin:"0 auto"}}>
        ...
        <div style={{color:WHITE,fontSize:11,marginBottom:20,letterSpacing:0.2}}>Effective march 2026 Â· mandastrong studio</div>
        ...
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",marginBottom:28,border:"1px solid "+GOLDDIM}}>
          {[["tos","TERMS OF SERVICE"],["disc","DISCLAIMER"]].map(([id,label])=>(
            <button key={id} onClick={()=>setTab(id)}
              style={{background:tab===id?"#1C1F25":"#000",border:"none",borderBottom:tab===id?"2px solid "+SIGNAL:"2px solid transparent",color:tab===id?GOLD:WHITE,padding:"14px",cursor:"pointer",fontSize:12,fontWeight:600,letterSpacing:0.2,fontFamily:"'Archivo',system-ui,sans-serif"}}>
              {label}
            </button>
          ))}
        </div>
        ...
        {tab==="tos"&&(
          <div>
            <div style={{background:"#14161A",border:"2px solid "+SIGNAL,padding:"14px 20px",marginBottom:20,textAlign:"center"}}>
              <div style={{color:GOLD,fontSize:11,letterSpacing:0.2,fontWeight:600}}>Mandastrong studio Â· professional cinema intelligence platform</div>
            ...
            <div style={{background:"#14161A",border:"1px solid "+GOLDDIM,padding:"12px 16px",marginTop:8}}>
              <p style={{color:GOLDDIM,fontSize:11,margin:0,letterSpacing:0}}>Mandastrong studio Â· amanda woolley, founder Â· march 2026</p>
            </div>
          </div>
        )}
        ...
        {tab==="disc"&&(
          <div>
            <div style={{background:"#14161A",border:"2px solid "+SIGNAL,padding:"14px 20px",marginBottom:20,textAlign:"center"}}>
              <div style={{color:GOLD,fontSize:11,letterSpacing:0.2,fontWeight:600}}>Important â please read before using this platform</div>
            ...
            <div style={{background:"#14161A",border:"1px solid "+GOLDDIM,padding:"12px 16px",marginTop:8}}>
              <p style={{color:GOLDDIM,fontSize:11,margin:0,letterSpacing:0}}>â Amanda Woolley Â· Founder Â· MandaStrong Studio Â· March 2026 Â· mandsstrongmovie.bolt.host</p>
            </div>
          </div>
        )}
      </div>
...
function P21() {
  ...
  return(
    <div style={{height:"calc(100vh - 116px)",display:"flex",flexDirection:"column",background:"#1C1F25",overflow:"hidden"}}>
      <div style={{flex:1,margin:"12px 16px",border:"2px solid "+SIGNAL,display:"flex",flexDirection:"column",background:"#14161A",overflow:"hidden",minHeight:0}}>
        <div style={{background:"#1C1F25",borderBottom:"2px solid "+SIGNAL,padding:"12px 18px",display:"flex",alignItems:"center",gap:12,flexShrink:0}}>
          ...
          <div style={{display:"flex",gap:5,flexShrink:0}}>
            {[["23","PAGES"],["600+","TOOLS"],["54","VOICES"],["4K","RENDER"]].map(([v,l])=>(
              <div key={l} style={{background:"#1C1F25",border:"1px solid "+GOLDDIM+"44",padding:"5px 8px",textAlign:"center",minWidth:40}}>
        ...
        <div style={{flex:1,overflowY:"auto",padding:"12px 16px",display:"flex",flexDirection:"column",gap:10,minHeight:0}}>
          {msgs.map((m,i)=>(
            <div key={i} style={{display:"flex",gap:10,flexDirection:m.role==="user"?"row-reverse":"row"}}>
              <div style={{width:32,height:32,flexShrink:0,background:m.role==="user"?"#22262D":GOLD,border:"1px solid "+GOLDDIM,display:"flex",alignItems:"center",justifyContent:"center",fontSize:12,fontWeight:600,color:m.role==="user"?GOLD:"#000",fontFamily:"'Archivo',system-ui,sans-serif"}}>{m.role==="user"?"Y":"G"}</div>
        ...
        <div style={{borderTop:"1px solid "+GOLDDIM+"22",padding:"8px 16px",display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(190px,1fr))",gap:4,flexShrink:0,background:"#040200"}}>
          {QUICK.map(q=>(
            <button key={q} onClick={()=>send(q)}
              style={{background:"#1C1F25",border:"1px solid "+GOLDDIM+"33",color:GOLDDIM,padding:"6px 10px",cursor:"pointer",fontSize:11,fontWeight:500,fontFamily:"'Archivo',system-ui,sans-serif",textAlign:"left",lineHeight:1.4}}
        ...
        <div style={{borderTop:"1px solid "+GOLDDIM,padding:"10px 16px",display:"flex",gap:8,flexShrink:0,background:"#14161A"}}>
          <textarea value={inp2} onChange={e=>setInp2(e.target.value)}
            ...
            rows={2}
            style={{flex:1,resize:"none",padding:"9px 12px",fontSize:13,background:"#1C1F25",border:"1px solid "+GOLDDIM+"44",color:WHITE,outline:"none",lineHeight:1.6,fontFamily:"'Archivo',system-ui,sans-serif",boxSizing:"border-box"}}/>
          <button onClick={()=>send()} disabled={loading||!inp2.trim()}
            style={{background:loading||!inp2.trim()?"#22262D":GOLD,border:"1px solid "+(loading||!inp2.trim()?GOLD+"22":GOLD),color:loading||!inp2.trim()?GOLDDIM:"#000",padding:"10px 20px",cursor:loading||!inp2.trim()?"not-allowed":"pointer",fontSize:12,fontWeight:600,letterSpacing:0.2,fontFamily:"'Archivo',system-ui,sans-serif",alignSelf:"stretch"}}>
            {loading?"â³":"Send"}
          </button>
        </div>
      </div>
        ...
        {posts.map(p=>(
          <div key={p.id} style={{...Card(),marginBottom:8,display:"flex",alignItems:"center",justifyContent:"space-between"}}>
            ...
            <div style={{display:"flex",alignItems:"center",gap:12}}>
              ...
              <span style={{color:WHITE,fontSize:12}}>â¤ {p.likes.toLocaleString()}</span>
...
function HowToGuide() {
  ...
  const SECTIONS=[
    ...
    {t:"Part two Â· Getting started",c:"Open mandsstrongmovie.bolt.host. Log in with your credentials or start a free trial. Use the hamburger menu top left to jump to any of the 24 pages. AUTOSAVE ON is real â your work saves automatically every time you change page, generate a clip, or update your timeline. Hit SAVE PROJECT to create a named restore point you can return to from MY PROJECTS. Your plan and remaining usage are always visible from your account panel â tap the avatar top right."},

    {t:"Part two Â· Page 1 â home & install",c:"The front door of mandsstrongmovie.bolt.host. The DOWNLOAD APP button installs the studio to your device like a real app, using your browser's built-in install prompt â on iPhone and iPad use Share then Add to Home Screen, as Apple does not allow one-tap install. The whole page is built to fit any screen, phone or laptop. From here, enter the studio and begin."},
    ...
    {t:"SAVING, RECOVERING & GETTING HELP",c:"AUTOSAVE ON saves as you work. SAVE PROJECT creates a named session â name it meaningfully. MY PROJECTS shows your history; CONTINUE PROJECT restores a session including all clips. An emergency save fires if the tab closes or crashes, so work is never permanently lost. Stuck? Agent Grok on Page 21 is your 24/7 production consultant with full knowledge of every page and workflow. This guide lives on your closing page at mandsstrongmovie.bolt.host and is updated as the studio grows."},

    {t:"Recommended workflow â start to finish",c:"Page 5 fill Script to Movie's Producer, Describe, Production boxes WIRE INTO RENDER. Page 6 choose a voice PREPARE TO SPEAK SAVE TO MEDIA LIBRARY. Page 8 upload a reference photo generate each scene (your brief drives them) add background music and stereo if you like. Page 13 SYNC ALL TRACKS. Page 15 set the mix. Page 16 choose quality render. Page 17 preview. Page 18 export and share. That is a finished film, made by you, at mandsstrongmovie.bolt.host."},
  ];
  return(
    <div style={{padding:"20px 32px 40px",maxWidth:860,margin:"0 auto"}}>
      <div style={{color:GOLD,fontWeight:600,fontSize:12,letterSpacing:0.4,marginBottom:12,textAlign:"center"}}>How to use mandastrong studio â click any section</div>
      {SECTIONS.map((g,i)=>{
        const isOpen=open===i;
        return(
          <div key={i} style={{marginBottom:4}}>
            <button onClick={()=>setOpen(isOpen?null:i)} style={{width:"100%",background:isOpen?GOLD+"14":"#14161A",border:"1px solid "+(isOpen?GOLD:GOLDDIM+"55"),padding:"13px 18px",display:"flex",justifyContent:"space-between",alignItems:"center",cursor:"pointer",fontFamily:"'Archivo',system-ui,sans-serif"}}>
              <span style={{color:isOpen?GOLD:WHITE,fontWeight:600,fontSize:13,letterSpacing:0.2}}>{g.t}</span>
              <span style={{color:GOLD,fontSize:16,fontWeight:600}}>{isOpen?"â²":"â¼"}</span>
            </button>
...
function P24CharacterStudio({ onSave, go }) {
  ...
  // ââ LIP-SYNC (real, via Replicate Wav2Lip through Supabase) ââ
  ...
  const lsRecord=async()=>{
    ...
    }catch(e){setLsError("Microphone blocked â allow mic access and try again.");}
  };
  const makeAvatarSpeak=async()=>{
    ...
    setLsBusy(true);setLsStage("Sending your avatar and voice to the lip-sync engineâ¦");
    try{
      const res=await fetch("https://njqfexhltjwpgvctmyaw.supabase.co/functions/v1/lip-sync",{
        method:"POST",headers:{"Content-Type":"application/json"},
  ...
  const useInScene=(c)=>{
    if(onSave&&c.photo){
      ...
      alert("â "+c.name+" reference image sent to your Media Library â upload it on Page 8 to keep this character consistent.");
    }else{
  ...
  const generatePrompt=(c)=>{
    ...
    navigator.clipboard.writeText(prompt).then(()=>alert("Scene prompt copied â paste into Page 8 scene description.")).catch(()=>alert(prompt));
  };

  const inp={width:"100%",background:"#1C1F25",border:"1px solid "+GOLDDIM,padding:"10px 12px",color:WHITE,fontSize:13,outline:"none",boxSizing:"border-box",fontFamily:"'Archivo',system-ui,sans-serif"};
  ...
  const optBtn=(val,cur,setter,opts)=>(
    <div style={{display:"flex",flexWrap:"wrap",gap:4,marginBottom:4}}>
      {opts.map(o=><button key={o} onClick={()=>setter(o)} style={{background:cur===o?GOLD:"#1C1F25",border:"1px solid "+(cur===o?"#000":GOLDDIM),color:cur===o?"#000":WHITE,padding:"3px 10px",cursor:"pointer",fontSize:11,fontWeight:600}}>{o}</button>)}
    </div>
  );
    ...
    <div style={{...Sp,padding:30}}>
      <div style={{maxWidth:1100,margin:"0 auto"}}>
        ...
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:24}}>
          ...
          <div style={{...Card(),border:"2px solid "+SIGNAL,maxHeight:"80vh",overflowY:"auto"}}>
            ...
            {photo?(
              <div style={{position:"relative",marginBottom:12}}>
                ...
                <button onClick={()=>{setPhoto(null);setPhotoName("");}} style={{position:"absolute",top:5,right:5,background:"#1C1F25",border:"1px solid "+GOLDDIM,color:GOLD,padding:"2px 8px",cursor:"pointer",fontSize:11,fontWeight:600}}>â</button>
              </div>
            ):(
              <div
                onClick={()=>fileRef.current&&fileRef.current.click()}
                onDragOver={e=>{e.preventDefault();e.currentTarget.style.background="#2E343D";}}
                onDragLeave={e=>{e.currentTarget.style.background="#1C1F25";}}
                onDrop={e=>{
                  e.preventDefault();e.currentTarget.style.background="#1C1F25";
                  const f=e.dataTransfer.files&&e.dataTransfer.files[0];
                ...
                style={{width:"100%",background:"#1C1F25",border:"2px dashed "+GOLD,color:GOLD,padding:"18px 14px",cursor:"pointer",fontSize:12,fontWeight:600,letterSpacing:0.2,fontFamily:"'Archivo',system-ui,sans-serif",marginBottom:12,textAlign:"center"}}>
            ...
            <select value={voice} onChange={e=>setVoice(e.target.value)} style={{...inp,marginBottom:14,cursor:"pointer"}}>
              {VOICE_CHARACTERS.map(v=><option key={v.id} value={v.id} style={{background:"#1C1F25"}}>{v.name} â {v.origin} {v.gender} Â· {v.style}</option>)}
            </select>
            ...
            {/* ââââââââ REAL LIP-SYNC â MAKE THIS AVATAR SPEAK ââââââââ */}
            <div style={{marginTop:16,border:"2px solid "+SIGNAL,background:"#1C1F25",padding:16,boxShadow:"none"}}>
              ...
              <div style={{display:"flex",gap:8,marginBottom:10,flexWrap:"wrap"}}>
                <button onClick={lsRecord} style={{flex:1,minWidth:140,background:lsRecording?"#c0392b":"#1C1F25",border:"1px solid "+(lsRecording?"#000":GOLDDIM),color:lsRecording?"#fff":WHITE,padding:"11px",cursor:"pointer",fontSize:12,fontWeight:600,letterSpacing:0,fontFamily:"'Archivo',system-ui,sans-serif"}}>{lsRecording?"Stop recording":"Record voice"}</button>
                <button onClick={()=>{const i=document.getElementById("lsAudioInput");if(i)i.click();}} style={{flex:1,minWidth:140,background:"#1C1F25",border:"1px solid "+GOLDDIM,color:WHITE,padding:"11px",cursor:"pointer",fontSize:12,fontWeight:600,letterSpacing:0,fontFamily:"'Archivo',system-ui,sans-serif"}}>â¤´ upload voice</button>
              ...
              <button onClick={makeAvatarSpeak} disabled={lsBusy} style={{width:"100%",padding:15,background:lsBusy?"#333":GOLD,color:lsBusy?"#888":"#000",border:"none",fontWeight:600,fontSize:15,letterSpacing:0.2,cursor:lsBusy?"default":"pointer",fontFamily:"'Archivo',system-ui,sans-serif"}}>{lsBusy?"GENERATINGâ¦":"Make avatar speak"}</button>
              ...
              {lsVideo&&(
                <div style={{marginTop:12}}>
                  <video src={lsVideo} controls autoPlay playsInline style={{width:"100%",border:"1px solid "+GOLDDIM,background:"#1C1F25"}}/>
          ...
          {/* Library panel */}
          <div>
            <div style={{color:GOLD,fontSize:12,letterSpacing:0.2,fontWeight:600,marginBottom:14}}>Your characters â {chars.length}</div>
            {chars.length===0?(
                ...
                {chars.map(c=>(
                  <div key={c.id} style={{...Card(),padding:14}}>
                    <div style={{display:"flex",gap:12,alignItems:"flex-start"}}>
                      {c.photo?<img src={c.photo} alt={c.name} style={{width:80,height:80,objectFit:"cover",border:"2px solid "+SIGNAL,flexShrink:0}}/>:<div style={{width:80,height:80,background:"#1C1F25",border:"1px solid "+GOLDDIM,display:"flex",alignItems:"center",justifyContent:"center",fontSize:28,flexShrink:0}}></div>}
                      <div style={{flex:1,minWidth:0}}>
                        ...
                        <div style={{color:WHITE,fontSize:11,marginTop:4,display:"flex",flexWrap:"wrap",gap:6}}>
                          {c.gender&&<span style={{background:"#1C1F25",border:"1px solid "+GOLDDIM,padding:"1px 6px",fontSize:10}}>{c.gender}</span>}
                          {c.age&&<span style={{background:"#1C1F25",border:"1px solid "+GOLDDIM,padding:"1px 6px",fontSize:10}}>{c.age}</span>}
                          {c.ethnicity&&<span style={{background:"#1C1F25",border:"1px solid "+GOLDDIM,padding:"1px 6px",fontSize:10}}>{c.ethnicity}</span>}
                          {c.hairColor&&<span style={{background:"#1C1F25",border:"1px solid "+GOLDDIM,padding:"1px 6px",fontSize:10}}>{c.hairStyle?c.hairStyle+" ":""}{c.hairColor} hair</span>}
                          {c.eyeColor&&<span style={{background:"#1C1F25",border:"1px solid "+GOLDDIM,padding:"1px 6px",fontSize:10}}>{c.eyeColor} eyes</span>}
                        </div>
                        ...
                        <div style={{color:WHITE,fontSize:11,marginTop:4}}>ð {VOICE_CHARACTERS.find(v=>v.id===c.voice)?.name||c.voice}</div>
                      </div>
                    </div>
...
function P23({ go }) {
  ...
  return(
    <div style={{...Sp,padding:0,background:"#1C1F25",position:"relative",minHeight:"100vh",overflow:"hidden"}}>
      <div style={{position:"relative",zIndex:1,padding:"30px 24px 80px"}}>
        <div style={{maxWidth:880,margin:"0 auto",textAlign:"center"}}>
          ...
          <div style={{fontSize:10,color:GOLD,letterSpacing:0.4,marginBottom:8,fontWeight:500}}>Mandastrong studio Â· cinema intelligence platform</div>
          ...
          <button onClick={()=>setHowOpen(o=>!o)} style={{width:"100%",background:howOpen?GOLD:"#050500ee",border:"2px solid "+SIGNAL,color:howOpen?"#000":GOLD,padding:"18px 24px",cursor:"pointer",fontFamily:"'Archivo',system-ui,sans-serif",fontSize:15,fontWeight:600,letterSpacing:0.4,marginBottom:howOpen?0:28,display:"flex",justifyContent:"space-between",alignItems:"center",boxShadow:"none"}}>
            <span>MANDASTRONG STUDIO â THE COMPLETE GUIDE &amp; AI HANDBOOK</span>
          ...
          <a href="https://MandaStrong1.Etsy.com" target="_blank" rel="noreferrer" style={{display:"block",background:GOLD,color:"#000",padding:"16px 24px",fontWeight:600,fontSize:14,letterSpacing:0.2,textDecoration:"none",marginBottom:28,fontFamily:"'Archivo',system-ui,sans-serif"}}>Humanity for future AI â mandastrong1.etsy.com</a>
...
// ââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââ
// ââ LANDING SCREEN â full-screen futuristic film video, name, one button ââ
  ...
  return (
    <div style={{position:"fixed",inset:0,zIndex:1000,background:"#0A0B0D",overflow:"hidden",display:"flex",alignItems:"center",justifyContent:"center"}}>
      ...
      <div style={{position:"relative",textAlign:"center",padding:"0 24px",maxWidth:760}}>
        <div style={{fontSize:11,letterSpacing:3.4,color:"#6FA8FF",marginBottom:18,fontWeight:600}}>CINEMA INTELLIGENCE PLATFORM</div>
        <div style={{fontSize:"clamp(34px,8vw,74px)",lineHeight:1.02,fontWeight:600,color:"#E9ECEF",letterSpacing:-0.5}}>MandaStrong Studio</div>
        <div style={{marginTop:16,fontSize:"clamp(13px,3.2vw,17px)",color:"#8B939C",lineHeight:1.5}}>Your story, made into a film.</div>
        <button onClick={onEnter} style={{marginTop:34,background:"#6FA8FF",color:"#0A0B0D",border:"none",borderRadius:10,padding:"16px 40px",fontSize:15,fontWeight:600,letterSpacing:0.4,cursor:"pointer",fontFamily:"'Archivo',system-ui,sans-serif"}}>Press to Create</button>
      </div>
    </div>
  );
}
...
function IntroDoors({ onEnter }){
  ...
  const metalLeft="#1C1F25";
  const metalRight="#1C1F25";
  return (
    <div style={{position:"fixed",inset:0,zIndex:100000,background:"#1C1F25",overflow:"hidden",
      ...
      <div style={{position:"absolute",left:0,right:0,bottom:"6%",display:"flex",flexDirection:"column",alignItems:"center",
        ...
        <div style={{color:GOLDDIM,fontSize:"clamp(8px,1.4vw,11px)",letterSpacing:0.2,marginTop:12,textAlign:"center",padding:"0 16px"}}>Cinema intelligence platform Â· 600+ AI tools Â· up to 3-hour films</div>
        ...
        <div style={{color:GOLDDIM,fontSize:11,letterSpacing:0.2,marginTop:16}}>mandsstrongmovie.bolt.host</div>
      </div>
    </div>
  );
}
...
export default function App() {
  ...
  return (
    <div style={{background:"#1C1F25",minHeight:"100vh",fontFamily:"'Archivo',system-ui,sans-serif"}}>
