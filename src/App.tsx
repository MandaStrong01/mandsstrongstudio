    ...
    <div style={{minHeight:"100vh",background:"#000",color:WHITE,fontFamily:"'Rajdhani',sans-serif",paddingBottom:160}}>
      ...
      <div style={{margin:"14px 20px",background:"linear-gradient(135deg,#0a0500,#160a00)",border:"2px solid "+GOLD,borderRadius:10,padding:16,boxShadow:"0 0 24px "+GOLD+"22"}}>
        <div style={{textAlign:"center",marginBottom:12}}>
          <div style={{fontFamily:"'Cinzel',serif",color:GOLD,letterSpacing:4,fontSize:22,textTransform:"uppercase"}}>âœ¦ MAKE MY MOVIE âœ¦</div>
          <div style={{color:DIM,fontSize:11,letterSpacing:2,marginTop:3}}>DROP EVERYTHING IN Â· SET THE LENGTH Â· PRESS GO â€” IT DOES THE REST</div>
        </div>
        ...
        <div ref={mmmDropRef}
          ...
          style={{textAlign:"center",border:"1px dashed "+GOLD+"77",borderRadius:6,padding:12,color:DIM,fontSize:12,letterSpacing:1,marginBottom:10,cursor:"pointer"}}>
          â¬† DROP OR TAP TO ADD SCRIPT &amp; IMAGES
          ...
          <div style={{display:"flex",flexWrap:"wrap",gap:5,marginBottom:10}}>
            {mmmImages.map((im,i)=>(
              <div key={i} style={{position:"relative"}}>
                <img src={im.dataUrl} style={{width:54,height:54,objectFit:"cover",border:"1px solid "+GOLD,borderRadius:4}}/>
                <button onClick={()=>setMmmImages(p=>p.filter((_,x)=>x!==i))}
                  style={{position:"absolute",top:-6,right:-6,width:18,height:18,borderRadius:9,background:"#000",color:GOLD,border:"1px solid "+GOLD,fontSize:11,cursor:"pointer",lineHeight:"16px",padding:0}}>Ã—</button>
              </div>
            ))}
          </div>
        )}
        ...
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:5}}>
          <span style={{color:GOLD,fontSize:12,letterSpacing:2,fontWeight:900}}>MOVIE LENGTH</span>
          <span style={{color:"#fff",fontSize:12,fontWeight:700}}>{mmmTargetMin} MIN Â· ~{Math.max(1,Math.round(mmmTargetMin))} SCENES</span>
        </div>
        <input type="range" min={1} max={180} value={mmmTargetMin}
          onChange={e=>setFilmDuration&&setFilmDuration(Number(e.target.value))}
          style={{width:"100%",accentColor:GOLD,marginBottom:2}}/>
        ...
        <button onClick={makeMyMovie} disabled={mmmBusy}
          ...
          {mmmBusy?"MAKING YOUR MOVIEâ€¦":"âœ¦ MAKE MY MOVIE"}
        </button>
        ...
        {mmmScenes.length>0&&(
          <div style={{marginTop:10,maxHeight:120,overflowY:"auto",fontSize:11}}>
            {mmmScenes.map((s,i)=>(
              <div key={i} style={{display:"flex",gap:6,padding:"2px 0",color:s.status==="done"?GOLD:s.status==="rendering"?"#fff":s.status==="skipped"?"#ff8a8a":DIM}}>
                <span>{s.status==="done"?"âœ“":s.status==="rendering"?"â—":s.status==="skipped"?"âœ•":"â—‹"}</span>
                <span style={{whiteSpace:"nowrap",overflow:"hidden",textOverflow:"ellipsis"}}>Scene {i+1}: {s.text}</span>
              </div>
            ))}
          </div>
      ...
      <div style={{maxWidth:860,margin:"0 auto"}}>
        ...
        {tab==="disc"&&(
          <div>
            ...
            <div style={{background:"#050500",border:"1px solid "+GOLDDIM,padding:"12px 16px",marginTop:8}}>
              <p style={{color:GOLDDIM,fontSize:11,margin:0,letterSpacing:1}}>â€” AMANDA WOOLLEY Â· FOUNDER Â· MANDASTRONG STUDIO Â· MARCH 2026 Â· mandastrongmovie.bolt.host</p>
            </div>
          </div>
        )}
      </div>
...
function HowToGuide() {
  const [open,setOpen]=useState(null);
  const SECTIONS=[
    {t:"WELCOME â€” HOW TO READ THIS BOOK",c:"This is more than a how-to. It is a complete guide to making films with AI on MandaStrong Studio (mandastrongmovie.bolt.host) AND a plain-English education in what AI actually is, so you are never at its mercy. Read Part One to understand the machine you are working with. Read Part Two to master the studio page by page. Read Part Three for the craft â€” prompting, voice, story, and ethics. You do not need any technical background. Every idea here is explained the way you would explain it to a friend across a kitchen table."},
    ...
    {t:"PART TWO Â· GETTING STARTED",c:"Open mandastrongmovie.bolt.host. Log in with your credentials or start a free trial. Use the â˜° hamburger menu top left to jump to any of the 24 pages. AUTOSAVE ON is real â€” your work saves automatically every time you change page, generate a clip, or update your timeline. Hit ðŸ’¾ SAVE PROJECT to create a named restore point you can return to from MY PROJECTS. Your plan and remaining usage are always visible from your account panel â€” tap the avatar top right."},

    {t:"PART TWO Â· PAGE 1 â€” HOME & INSTALL",c:"The front door of mandastrongmovie.bolt.host. The DOWNLOAD APP button installs the studio to your device like a real app, using your browser's built-in install prompt â€” on iPhone and iPad use Share then Add to Home Screen, as Apple does not allow one-tap install. The whole page is built to fit any screen, phone or laptop. From here, enter the studio and begin."},
        ...
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:24}}>
          ...
          <div style={{...Card(),border:"2px solid "+GOLD,maxHeight:"80vh",overflowY:"auto"}}>
            ...
            <div style={{marginTop:16,border:"2px solid "+GOLD,background:"linear-gradient(135deg,#0a0500,#1a0a00)",padding:16,boxShadow:"0 0 24px "+GOLD+"22"}}>
              <div style={{fontFamily:"'Cinzel',serif",color:GOLD,letterSpacing:3,fontSize:16,textTransform:"uppercase",marginBottom:4}}>âœ¦ Make This Avatar Speak</div>
              ...
              <div style={{display:"flex",gap:8,marginBottom:10,flexWrap:"wrap"}}>
                <button onClick={lsRecord} style={{flex:1,minWidth:140,background:lsRecording?"#c0392b":"#111",border:"1px solid "+(lsRecording?"#000":GOLDDIM),color:lsRecording?"#fff":WHITE,padding:"11px",cursor:"pointer",fontSize:12,fontWeight:900,letterSpacing:1,fontFamily:"'Rajdhani',sans-serif"}}>{lsRecording?"â–  STOP RECORDING":"â— RECORD VOICE"}</button>
                <button onClick={()=>{const i=document.getElementById("lsAudioInput");if(i)i.click();}} style={{flex:1,minWidth:140,background:"#111",border:"1px solid "+GOLDDIM,color:WHITE,padding:"11px",cursor:"pointer",fontSize:12,fontWeight:900,letterSpacing:1,fontFamily:"'Rajdhani',sans-serif"}}>â¤´ UPLOAD VOICE</button>
              ...
              {lsVideo&&(
                <div style={{marginTop:12}}>
                  ...
                  <button onClick={lsDownload} style={{width:"100%",marginTop:8,padding:13,background:"linear-gradient(135deg,"+GOLDDIM+","+GOLD+")",color:"#000",border:"none",fontWeight:900,fontSize:14,letterSpacing:2,cursor:"pointer",fontFamily:"'Cinzel',serif"}}>â¬‡ DOWNLOAD LIP-SYNC VIDEO</button>
                </div>
              )}
            </div>
